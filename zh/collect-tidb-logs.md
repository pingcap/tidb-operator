...
如果希望将慢查询日志和应用日志区分开，可以在 `TidbCluster` CR 中配置 `spec.tidb.separateSlowLog: true` 参数，将慢查询日志输出到一个专用的旁路容器中，这样慢查询日志在宿主机上会被输出到一个单独的文件，和应用日志分开。

然后可以通过名为 `slowlog` 的 sidecar 容器查看慢查询日志：
