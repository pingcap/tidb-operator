...
func addIndexer(ctx context.Context, mgr ctrl.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &v1alpha1.SchedulingGroup{}, "spec.cluster.name", func(obj client.Object) []string {
...
func setupControllers(mgr ctrl.Manager, c client.Client, pdcm pdm.PDClientManager, vm volumes.ModifierFactory) error {
	setups := []controllerSetup{
		...
		},
		{
			...
			setupFunc: func() error {
				return schedulinggroup.Setup(mgr, c)
			},
		},
		{
			name: "Scheduling",
			setupFunc: func() error {
				return scheduling.Setup(mgr, c, pdcm, vm)
			},
		},
		{
			name: "TiProxyGroup",
