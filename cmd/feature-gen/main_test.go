...
func TestRebuildAndCheck(t *testing.T) {
	...
	apiSource, err := os.ReadFile(filepath.Join(root, "api", "meta", "v1alpha1", "zz_generated.features.go"))
	if err != nil {
		t.Fatal(err)
	}
	if bytes.Contains(apiSource, []byte("ignore_autogenerated")) {
		t.Fatal("generated API declarations must remain visible to deepcopy-gen")
	}
