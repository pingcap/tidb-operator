...
func (bo *Options) doTruncateLogBackup(ctx context.Context, backup *v1alpha1.Backup) error {
	...
	// Default behavior: not add --clean-up-compactions unless explicitly set to true
	if value, exists := backup.Annotations["backup.pingcap.com/clean-up-compactions"]; exists && value == "true" {
