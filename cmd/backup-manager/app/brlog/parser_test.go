...
func TestParseLineIgnoresJSONOperationStarted(t *testing.T) {
	...
	if event.Type != EventNone || event.Operation != nil {
		t.Fatalf("expected JSON operation log to be ignored, got %#v", event)
	}
}
...
func TestParseLineTextOperationStartedWithQuotedBracketValue(t *testing.T) {
	...
	if event.Operation.OperationID != "op-a" {
		t.Fatalf("unexpected operation id: %q", event.Operation.OperationID)
	}
	if event.Operation.Command != "log truncate [manual]" {
		t.Fatalf("unexpected command: %q", event.Operation.Command)
	}
}
...
func TestParseLineIgnoresJSONMessageFallbacks(t *testing.T) {
	tests := []struct {
		name string
		line string
	}{
