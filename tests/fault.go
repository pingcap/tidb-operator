...
func (fa *faultTriggerActions) CheckAndRecoverEnv() error {
	k8se2elog.Logf("ensure all nodes are running")
	...
	k8se2elog.Logf("ensure all etcds are running")
	err := fa.StartETCD()
	if err != nil {
		return err
	}
	...
	k8se2elog.Logf("ensure all kubelets are running")
	...
	k8se2elog.Logf("ensure all static pods are running")
	...
	k8se2elog.Logf("ensure all kube-proxy are running")
...
func (fa *faultTriggerActions) StopNode() (string, string, time.Time, error) {
	...
	k8se2elog.Logf("selecting %s as the node to failover", node)
	...
	k8se2elog.Logf("node %s on physical node %s is stopped", node, physicalNode)
...
func (fa *faultTriggerActions) StartNode(physicalNode string, node string) error {
	...
	k8se2elog.Logf("node %s on physical node %s is started", node, physicalNode)
...
func (fa *faultTriggerActions) StopKubeProxy() error {
	k8se2elog.Logf("stopping all kube-proxy pods")
	...
	for _, pod := range pods {
		k8se2elog.Logf("waiting for kube-proxy pod %s/%s to be terminated", pod.Namespace, pod.Name)
		err = waitForPodNotFoundInNamespace(fa.kubeCli, pod.Name, pod.Namespace, PodTimeout)
		if err != nil {
			return err
		}
	}
	k8se2elog.Logf("kube-proxy on vm nodes %v are stopped", nodes)
	return nil
}
...
func (fa *faultTriggerActions) StartKubeProxy() error {
	k8se2elog.Logf("starting all kube-proxy pods")
	nodes := getAllK8sNodes(fa.cfg)
	ds, err := fa.kubeCli.AppsV1().DaemonSets(metav1.NamespaceSystem).Get("kube-proxy", metav1.GetOptions{})
	if err != nil {
		return err
	...
	k8se2elog.Logf("kube-proxy on vm nodes %v are started", nodes)
	return nil
}
...
func (fa *faultTriggerActions) StopETCDOrDie(nodes ...string) {
	k8se2elog.Logf("stopping %v etcds", nodes)
...
func (fa *faultTriggerActions) StopKubeletOrDie(nodes ...string) {
	k8se2elog.Logf("stopping %v kubelets", nodes)
...
func (fa *faultTriggerActions) serviceAction(node string, serverName string, action string) error {
	...
	k8se2elog.Logf("%s %s %s successfully", action, serverName, node)
