...
func (ctu *CrdTestUtil) tikvMembersReadyFn(obj runtime.Object) (bool, error) {
	...
	if tikvStatus.StatefulSet == nil {
		k8se2elog.Logf("%s/%s .status.StatefulSet is nil", ns, name)
		return false, nil
	}
	...
	if storeCounts != replicas {
		k8se2elog.Logf("%s/%s .status.TiKV.Stores.count(%d) != %d",
	...
	for _, store := range stores {
		if store.State != v1alpha1.TiKVStateUp {
			k8se2elog.Logf("%s/%s's store(%s) state != %s", ns, name, store.ID, v1alpha1.TiKVStateUp)
			return false, nil
		}
	}
...
func (ctu *CrdTestUtil) pumpMembersReadyFn(tc *v1alpha1.TidbCluster) (bool, error) {
	k8se2elog.Logf("begin to check incremental backup cluster[%s] namespace[%s]", tc.Name, tc.Namespace)
	...
	for _, pod := range pods.Items {
		...
		k8se2elog.Logf("pod.Spec.Affinity: %v", pod.Spec.Affinity)
		...
		k8se2elog.Logf("pod.Spec.Tolerations: %v", pod.Spec.Tolerations)
		foundKey := false
