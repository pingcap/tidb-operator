...
var _ = ginkgo.Describe("DMCluster", func() {
	...
	ginkgo.Context("[Feature:DM]", func() {
		ginkgo.It("basic feature for DM", func() {
			...
			k8se2e.ExpectNoError(err, "failed to create DmCluster: %q", dcName)
			k8se2e.ExpectNoError(oa.WaitForDmClusterReady(dc, 30*time.Minute, 30*time.Second), "failed to wait for DmCluster %q ready", dcName)
			...
			k8se2e.ExpectNoError(err, "failed to get TidbCluster for DM E2E tests")
			...
			k8se2e.ExpectNoError(err, "failed to deploy TidbMonitor for DmCluster %q", dcName)
			k8se2e.ExpectNoError(tests.CheckTidbMonitor(tm, cli, c, fw), "failed to check TidbMonitor for DmCluster %q", dcName)

			ginkgo.By("Create MySQL sources")
			k8se2e.ExpectNoError(tests.CreateDMSources(fw, dc.Namespace, controller.DMMasterMemberName(dcName)), "failed to create sources for DmCluster %q", dcName)
			...
			k8se2e.ExpectNoError(err, "failed to enable relay log for DmCluster %q", dcName)

			ginkgo.By("Generate full stage data in upstream")
			k8se2e.ExpectNoError(tests.GenDMFullData(fw, dc.Namespace), "failed to generate full stage data in upstream")

			ginkgo.By("Start a basic migration task")
			k8se2e.ExpectNoError(tests.StartDMTask(fw, dc.Namespace, controller.DMMasterMemberName(dcName), tests.DMSingleTask, ""), "failed to start single source task")

			ginkgo.By("Check data for full stage")
			k8se2e.ExpectNoError(tests.CheckDMData(fw, dc.Namespace, 1), "failed to check full data")

			ginkgo.By("Generate incremental stage data in upstream")
			k8se2e.ExpectNoError(tests.GenDMIncrData(fw, dc.Namespace), "failed to generate incremental stage data in upstream")

			ginkgo.By("Check data for incremental stage")
			k8se2e.ExpectNoError(tests.CheckDMData(fw, dc.Namespace, 1), "failed to check incremental data")

			ginkgo.By("Check custom labels and annotations")
			checkCustomLabelAndAnn(dc, c)

			ginkgo.By("Delete the dc")
			k8se2e.ExpectNoError(cli.PingcapV1alpha1().DMClusters(dc.Namespace).Delete(context.TODO(), dcName, metav1.DeleteOptions{}), "failed to delete dc %q", dcName)

			ginkgo.By("Wait for DmCluster to be deleted")
			k8se2e.ExpectNoError(oa.WaitForDmClusterDeleted(ns, dcName, 5*time.Minute, 10*time.Second), "failed to wait for DmCluster %q to be deleted", dcName)
		})
		...
		ginkgo.It("scale out with shard task for DM", func() {
			...
			k8se2e.ExpectNoError(tests.StartDMTask(fw, dc.Namespace, controller.DMMasterMemberName(dcName), tests.DMShardTask, "have not bound"), "failed to start shard source task before scale out")
			...
			k8se2e.ExpectNoError(err, "failed to scale out DmCluster: %q", dc.Name)
			k8se2e.ExpectNoError(oa.WaitForDmClusterReady(dc, 30*time.Minute, 30*time.Second), "failed to wait for DmCluster %q ready", dcName)

			ginkgo.By("Start a shard migration task after scale out")
			k8se2e.ExpectNoError(tests.StartDMTask(fw, dc.Namespace, controller.DMMasterMemberName(dcName), tests.DMShardTask, ""), "failed to start shard source task after scale out")

			ginkgo.By("Check data for full stage")
			k8se2e.ExpectNoError(tests.CheckDMData(fw, dc.Namespace, 2), "failed to check full data")

			ginkgo.By("Generate incremental stage data in upstream")
			k8se2e.ExpectNoError(tests.GenDMIncrData(fw, dc.Namespace), "failed to generate incremental stage data in upstream")

			ginkgo.By("Check data for incremental stage")
			k8se2e.ExpectNoError(tests.CheckDMData(fw, dc.Namespace, 2), "failed to check incremental data")
		})
		...
		ginkgo.It("scale in with shard task for DM", func() {
			...
			k8se2e.ExpectNoError(tests.StartDMTask(fw, dc.Namespace, controller.DMMasterMemberName(dcName), tests.DMShardTask, ""), "failed to start shard source task before scale in")
			...
			k8se2e.ExpectNoError(err, "failed to scale in DmCluster: %q", dc.Name)
			k8se2e.ExpectNoError(oa.WaitForDmClusterReady(dc, 30*time.Minute, 30*time.Second), "failed to wait for DmCluster %q ready", dcName)
			...
			ginkgo.By("Check data for incremental stage")
			k8se2e.ExpectEqual(tests.CheckDMData(fw, dc.Namespace, 2), wait.ErrWaitTimeout, "incremental data should not equal")
		})
		...
		ginkgo.It("restart pods for DM", func() {
			...
			for i := int32(0); i < dc.Spec.Master.Replicas; i++ {
				k8se2e.ExpectNoError(c.CoreV1().Pods(ns).Delete(context.TODO(), fmt.Sprintf("%s-%d", controller.DMMasterMemberName(dcName), i), metav1.DeleteOptions{}), "failed to delete DM-master pod %d for DmCluster %q", i, dcName)
				<-time.After(time.Minute) // wait the previous pod to be deleted
				k8se2e.ExpectNoError(oa.WaitForDmClusterReady(dc, 5*time.Minute, 30*time.Second), "failed to wait for DmCluster %q ready", dcName)
			}

			ginkgo.By("Restart the DM-worker pod")
			k8se2e.ExpectNoError(c.CoreV1().Pods(ns).Delete(context.TODO(), fmt.Sprintf("%s-0", controller.DMWorkerMemberName(dcName)), metav1.DeleteOptions{}), "failed to delete the DM-worker pod for DmCluster %q", dcName)
			<-time.After(time.Minute) // wait the previous pod to be deleted
			k8se2e.ExpectNoError(oa.WaitForDmClusterReady(dc, 5*time.Minute, 30*time.Second), "failed to wait for DmCluster %q ready", dcName)
		...
		ginkgo.It("change config with dmctl for DM", func() {
			...
			k8se2e.ExpectNoError(err, "failed to show source config for DmCluster %q: %v", dcName, err)
			k8se2e.ExpectEqual(strings.Contains(stdout, "enable-gtid: true"), true, "original source config doesn't contain `enable-gtid: true`")
			...
			k8se2e.ExpectNoError(ioutil.WriteFile(filename, []byte(cfg), 0o644), "failed to write updated source config file")
			_, err = framework.RunKubectl(ns, "cp", filename, fmt.Sprintf("%s/%s:%s", ns, podName, filename))
			k8se2e.ExpectNoError(err, "failed to copy source file into dm-master pod")
			...
			k8se2e.ExpectNoError(err, "failed to stop source for DmCluster %q", dcName)

			ginkgo.By("Create MySQL source again")
			_, err = framework.RunHostCmd(ns, podName,
				fmt.Sprintf("/dmctl --master-addr=127.0.0.1:8261 operate-source create %s", filename))
			k8se2e.ExpectNoError(err, "failed to create source again for DmCluster %q", dcName)

			ginkgo.By("Check source config after updated")
			stdout, err = framework.RunHostCmd(ns, podName,
				fmt.Sprintf("/dmctl --master-addr=127.0.0.1:8261 get-config source %s", sourceName))
			k8se2e.ExpectNoError(err, "failed to show source config after updated for DmCluster %q: %v", dcName, err)
			k8se2e.ExpectEqual(strings.Contains(stdout, "enable-gtid: false"), true, "original source config doesn't contain `enable-gtid: false`")
		})
		...
		ginkgo.It("deploy DM with TLS enabled", func() {
			...
			k8se2e.ExpectNoError(tidbcluster.InstallTiDBIssuer(ns, dcName), "failed to install CA certificate")

			ginkgo.By("Install MySQL certificate")
			k8se2e.ExpectNoError(tidbcluster.InstallMySQLCertificates(ns, dcName), "failed to install MySQL certificate")
			...
			k8se2e.ExpectNoError(tests.DeployDMMySQLWithTLSEnabled(c, ns, mysqlSecretName), "failed to deploy MySQL server with TLS enabled")

			ginkgo.By("Get MySQL TLS secret")
			mysqlSecret, err := c.CoreV1().Secrets(ns).Get(context.TODO(), mysqlSecretName, metav1.GetOptions{})
			k8se2e.ExpectNoError(err, "failed to get MySQL TLS secret")

			ginkgo.By("Check MySQL with TLS enabled ready")
			k8se2e.ExpectNoError(tests.CheckDMMySQLReadyWithTLSEnabled(fw, ns, mysqlSecret), "failed to wait for MySQL ready")

			ginkgo.By("Install TiDB server and client certificate")
			k8se2e.ExpectNoError(tidbcluster.InstallTiDBCertificates(ns, dcName), "failed to install TiDB server and client certificate")
			...
			k8se2e.ExpectNoError(tidbcluster.InstallDMCertificates(ns, dcName), "failed to install DM components certificate")
			<-time.After(30 * time.Second)
...
`, mysqlSecretName)
			...
			k8se2e.ExpectNoError(ioutil.WriteFile(filename, []byte(sourceCfg), 0o644), "failed to write source config file")
			_, err = framework.RunKubectl(ns, "cp", filename, fmt.Sprintf("%s/%s:%s", ns, podName, filename))
			k8se2e.ExpectNoError(err, "failed to copy source file into dm-master pod")
			_, err = framework.RunHostCmd(ns, podName,
				fmt.Sprintf("/dmctl --master-addr=127.0.0.1:8261 --ssl-ca=%s --ssl-cert=%s --ssl-key=%s operate-source create %s",
					...
					filename))
			k8se2e.ExpectNoError(err, "failed to create source for DmCluster %q", dcName)

			ginkgo.By("Generate full stage data in upstream")
			k8se2e.ExpectNoError(tests.GenDMFullDataWithMySQLNamespaceWithTLSEnabled(fw, ns, ns, mysqlSecret), "failed to generate full stage data in upstream")
...
`, tidbClientSecretName))
			...
			k8se2e.ExpectNoError(ioutil.WriteFile(filename, []byte(taskCfg), 0o644), "failed to write task config file")
			_, err = framework.RunKubectl(ns, "cp", filename, fmt.Sprintf("%s/%s:%s", ns, podName, filename))
			k8se2e.ExpectNoError(err, "failed to copy task file into dm-master pod")
			_, err = framework.RunHostCmd(ns, podName,
				fmt.Sprintf("/dmctl --master-addr=127.0.0.1:8261 --ssl-ca=%s --ssl-cert=%s --ssl-key=%s start-task %s",
					...
					"/var/lib/dm-master-tls/tls.key",
					filename))
			k8se2e.ExpectNoError(err, "failed to start single source task for DmCluster %q", dcName)
			...
			k8se2e.ExpectNoError(err, "failed to get TiDB client TLS secret")

			ginkgo.By("Check data for full stage")
			k8se2e.ExpectNoError(tests.CheckDMDataWithTLSEnabled(fw, ns, ns, ns, dcName, 1, mysqlSecret, tidbClientSecret), "failed to check full data")

			ginkgo.By("Generate incremental stage data in upstream")
			k8se2e.ExpectNoError(tests.GenDMIncrDataWithMySQLNamespaceWithTLSEnabled(fw, ns, ns, mysqlSecret), "failed to generate incremental stage data in upstream")

			ginkgo.By("Check data for incremental stage")
			k8se2e.ExpectNoError(tests.CheckDMDataWithTLSEnabled(fw, ns, ns, ns, dcName, 1, mysqlSecret, tidbClientSecret), "failed to check incremental data")
		})
		...
		ginkgo.It("kill pods for DM", func() {
			...
			for _, podName := range podNames {
				...
				k8se2e.ExpectNoError(err, "failed to get pod %q for DmCluster %q", podName, dcName)

				err = c.CoreV1().Pods(ns).Delete(context.TODO(), podName, metav1.DeleteOptions{})
				k8se2e.ExpectNoError(err, "failed to kill pod %q", podName)
				...
				k8se2e.ExpectNoError(err, "pod have not been recreated")

				err = wait.Poll(10*time.Second, 3*time.Minute, func() (done bool, err error) {
					return checkPodHealthy(podName), nil
				})
				k8se2e.ExpectNoError(err, "failed to wait for pod %q become healthy after restarted", podName)
			}
...
`
			...
			k8se2e.ExpectNoError(err, "failed to patch pod %q with invalid image for DmCluster %q", masterPodName, dcName)
			...
			k8se2e.ExpectNoError(err, "failed to patch pod %q with invalid image for DmCluster %q", workerPodName, dcName)
			...
			k8se2e.ExpectNoError(err, "failed to wait for %q become unhealthy for DmCluster %q", masterPodName, dcName)
			...
			k8se2e.ExpectNoError(err, "failed to wait for %q become offline for DmCluster %q", workerPodName, dcName)
			...
			k8se2e.ExpectNoError(err, "failed to wait for %q to be replaced after auto-failover and recover-failover for DmCluster %q", masterPodName, dcName)
			...
			k8se2e.ExpectNoError(err, "failed to wait for new dm-worker member to be created for DmCluster %q", dcName)

			ginkgo.By("Kill the old dm-worker pod")
			k8se2e.ExpectNoError(c.CoreV1().Pods(ns).Delete(context.TODO(), workerPodName, metav1.DeleteOptions{}), "failed to delete the pod %q for DmCluster", workerPodName, dcName)
			...
			k8se2e.ExpectEqual(err, wait.ErrWaitTimeout, "some dm-worker members have been deleted for DmCluster %q", dcName)
			...
			k8se2e.ExpectNoError(err, "failed to enable dm-worker RecoverFailover for DmCluster: %q", dc.Name)
			...
			k8se2e.ExpectNoError(err, "failed to wait for new created dm-worker for AutoFailover to be deleted for DmCluster: %q", dcName)
		})
		...
		ginkgo.It("upgrade for DM", func() {
			...
			checkImage := func(image string) {
				...
				k8se2e.ExpectNoError(err, "failed to get sts for %s/%s", ns, masterStsName)
				k8se2e.ExpectEqual(masterSts.Spec.Template.Spec.Containers[0].Image, image, "master sts image should be %q", image)
				...
				k8se2e.ExpectNoError(err, "failed to get sts for %s/%s", ns, workerStsName)
				k8se2e.ExpectEqual(workerSts.Spec.Template.Spec.Containers[0].Image, image, "worker sts image should be %q", image)
			}
			...
			k8se2e.ExpectNoError(err, "failed to pause DmCluster %q", dcName)
			...
			k8se2e.ExpectNoError(err, "failed to upgrade DmCluster %q", dcName)

			ginkgo.By("Check pods unchanged")
			err = utilpod.WaitForPodsAreChanged(c, podListPrev.Items, 3*time.Minute)
			k8se2e.ExpectEqual(err, wait.ErrWaitTimeout, "pods changed when DmCluster %q is paused", dcName)
			...
			k8se2e.ExpectNoError(err, "failed to resume DmCluster %q", dcName)

			ginkgo.By("Check pods changed")
			err = utilpod.WaitForPodsAreChanged(c, podListPrev.Items, 10*time.Minute)
			k8se2e.ExpectNoError(err, "failed to wait for pods changed for DmCluster %q", dcName)
			...
			k8se2e.ExpectNoError(err, "failed to downgrade dm-worker for DmCluster %q", dcName)

			ginkgo.By("Check dm-worker pods changed")
			err = utilpod.WaitForPodsAreChanged(c, workerPods.Items, 10*time.Minute)
			k8se2e.ExpectNoError(err, "failed to wait for dm-worker pods changed for DmCluster %q", dcName)

			ginkgo.By("Check dm-master pods unchanged")
			err = utilpod.WaitForPodsAreChanged(c, masterPods.Items, 3*time.Minute)
			k8se2e.ExpectEqual(err, wait.ErrWaitTimeout, "dm-master pods changed when not upgrading for DmCluster %q", dcName)
			...
			k8se2e.ExpectNoError(err, "failed to downgrade dm-master for DmCluster %q", dcName)

			ginkgo.By("Check dm-master pods changed")
			err = utilpod.WaitForPodsAreChanged(c, masterPods.Items, 10*time.Minute)
			k8se2e.ExpectNoError(err, "failed to wait for dm-master pods changed for DmCluster %q", dcName)
		})
	})
})
...
func checkCustomLabelAndAnn(dc *v1alpha1.DMCluster, c clientset.Interface) {
	...
	list, err = c.CoreV1().Pods(dc.Namespace).List(context.TODO(), listOptions)
	k8se2e.ExpectNoError(err)
	k8se2e.ExpectNotEqual(len(list.Items), 0, "expect dm-master pod exists")
	for _, pod := range list.Items {
		_, ok := pod.Labels[fixture.ClusterCustomKey]
		k8se2e.ExpectEqual(ok, true)
	...
	k8se2e.ExpectNoError(err)
	k8se2e.ExpectNotEqual(len(svcList.Items), 0, "expect dm-master svc exists")
	...
	list, err = c.CoreV1().Pods(dc.Namespace).List(context.TODO(), listOptions)
	k8se2e.ExpectNoError(err)
	k8se2e.ExpectNotEqual(len(list.Items), 0, "expect dm-worker pod exists")
	for _, pod := range list.Items {
		_, ok := pod.Labels[fixture.ClusterCustomKey]
		k8se2e.ExpectEqual(ok, true)
