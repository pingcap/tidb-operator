...
func registerTiProxyMySQLCerts(ctx context.Context, f *factory, ns, cluster string) error {
	...
	for _, g := range gs {
		...
		s, err := newCertKeyPair(typeTiProxyMySQLServer, ns, certKeyPair, cluster, g.GetName(), scope.Component[scope.TiProxyGroup](), false, false)
		if err != nil {
			return err
		}
...
func registerTiDBMySQLCerts(ctx context.Context, f *factory, ns, cluster string) error {
	...
	for _, g := range gs {
		...
		s, err := newCertKeyPair(typeTiDBMySQLServer, ns, certKeyPair, cluster, g.GetName(), scope.Component[scope.TiDBGroup](), false, false)
		if err != nil {
			return err
		}
		if err := f.RegisterSecret(s); err != nil {
...
func registerCertsForComponents[
	...
	for _, g := range gs {
		...
		s, err := newCertKeyPair(typeCluster, ns, certKeyPair, cluster, g.GetName(), scope.Component[S](), true, false)
		if err != nil {
			return err
		}
		if err := f.RegisterSecret(s); err != nil {
