...
func WaitObjectToBeControlledByOrDie(c client.Client, obj runtime.Object, owner runtime.Object, timeout time.Duration) {
	...
	err := wait.PollImmediate(10*time.Second, timeout, func() (bool, error) {
		...
		if err != nil {
			e2elog.Logf("error get object %s/%s: %v", objKind, meta.GetName(), err)
			return false, nil
		}
		...
		if !metav1.IsControlledBy(meta, ownerMeta) {
			e2elog.Logf("wait object %s/%s to be controlled by %s/%s...", objKind, meta.GetName(), ownerKind, ownerMeta.GetName())
			return false, nil
		}
		return true, nil
	})
	if err != nil {
		e2elog.Failf("error object %s/%s to be controlled by %s/%s: %v", objKind, meta.GetName(), ownerKind, ownerMeta.GetName(), err)
	}
}
