...
func WaitObjectToBeControlledByOrDie(c client.Client, obj runtime.Object, owner runtime.Object, timeout time.Duration) {
	...
	if !ok {
		k8se2elog.Failf("object is not a metav1.Object, cannot call WaitObjectToBeControlledByOrDie")
	}
	...
	if !ok {
		k8se2elog.Failf("owner is not a metav1.Object, cannot call WaitObjectToBeControlledByOrDie")
	}
	...
	err = wait.PollImmediate(10*time.Second, timeout, func() (bool, error) {
		...
		if err != nil {
			k8se2elog.Logf("error get object %s/%s: %v", objGVK.Kind, meta.GetName(), err)
			return false, nil
		}
		...
		if !metav1.IsControlledBy(meta, ownerMeta) {
			k8se2elog.Logf("wait object %s/%s to be controlled by %s/%s...", objGVK.Kind, meta.GetName(), ownerGVK.Kind, ownerMeta.GetName())
			return false, nil
		}
		return true, nil
	})
	if err != nil {
		k8se2elog.Failf("error object %s/%s to be controlled by %s/%s: %v", objGVK.Kind, meta.GetName(), ownerGVK.Kind, ownerMeta.GetName(), err)
	}
}
