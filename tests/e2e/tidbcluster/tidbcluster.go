...
var _ = ginkgo.Describe("TiDBCluster", func() {
	...
	ginkgo.Describe("when deploy, scale, update config", func() {
		clusterCfgs := []struct {
			Version string
			Name    string
		...
		for _, clusterCfg := range clusterCfgs {
			...
			ginkgo.It(fmt.Sprintf("should work in %s", localCfg.Version), func() {
				...
				framework.ExpectNoError(err, "failed to scale in TidbCluster: %+v", tc)
	...
	ginkgo.It("should keep tidb service in sync", func() {
		...
		framework.ExpectNoError(err, "failed to update tidb service of TidbCluster: %+v", tc)
	...
	ginkgo.It("should adopt helm created pump with TidbCluster CR", func() {
		...
		framework.ExpectNoError(err, "failed to wait for pump to be controlled by TidbCluster: %+v", tc)
	})
		...
		err = wait.PollImmediate(5*time.Second, 3*time.Minute, func() (bool, error) {
			for setName := range setNameToRevision {
				...
				if !metav1.IsControlledBy(usingCm, tc) {
					log.Logf("configmap of %s not controlled by tidbcluster, still waiting...", setName)
					return false, nil
				}
			}
			return true, nil
		})
		framework.ExpectNoError(err, "failed to waite for configmaps to be controlled by TidbCluster")
	})
	...
	ginkgo.It("can be paused and resumed", func() {
		...
		framework.ExpectNoError(err, "failed to pause TidbCluster: %+v", tc)
		...
		framework.ExpectNoError(err, "failed to upgrade TidbCluster version: %+v", tc)
		...
		framework.ExpectNoError(err, "failed to resume TidbCluster: %+v", tc)
