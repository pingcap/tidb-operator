...
var _ = ginkgo.Describe("[Serial]", func() {
	...
	ginkgo.Describe("[Feature: PodAdmissionWebhook]", func() {
		...
		ginkgo.It(fmt.Sprintf("should be able to upgrade TiDB Cluster from %s to %s", utilimage.TiDBVLatestPrev, utilimage.TiDBVLatest), func() {
			...
			ginkgo.By(fmt.Sprintf("start initial TidbCluster %q", utilimage.TiDBVLatestPrev))
			tc := fixture.GetTidbCluster(ns, "admission", utilimage.TiDBVLatestPrev)
			...
			ginkgo.By(fmt.Sprintf("Upgrade TidbCluster version to %q", utilimage.TiDBVLatest))
			err = controller.GuaranteedUpdate(genericCli, tc, func() error {
				tc.Spec.Version = utilimage.TiDBVLatest
				return nil
			})
			framework.ExpectNoError(err, "failed to update TidbCluster to upgrade tidb version to %v", utilimage.TiDBVLatest)

			ginkgo.By(fmt.Sprintf("wait for tikv-1 pod upgrading to %q", utilimage.TiDBVLatest))
			err = wait.Poll(5*time.Second, 10*time.Minute, func() (done bool, err error) {
				...
				if tikvPod.Spec.Containers[0].Image != fmt.Sprintf("pingcap/tikv:%s", utilimage.TiDBVLatest) {
					return false, nil
				}
				return true, nil
			})
			framework.ExpectNoError(err, "failed to upgrade tikv-1 to %q", utilimage.TiDBVLatest)
	...
	ginkgo.Describe("[Feature: Defaulting and Validating]", func() {
		...
		ginkgo.It("should perform defaulting and validating properly", func() {
				...
				Spec: v1alpha1.TidbClusterSpec{
					TiDB: &v1alpha1.TiDBSpec{
						...
						ComponentSpec: v1alpha1.ComponentSpec{
							Image: fmt.Sprintf("pingcap/tidb:%s", utilimage.TiDBVLatestPrev),
						},
					},
					TiKV: &v1alpha1.TiKVSpec{
						Replicas: 1,
						ComponentSpec: v1alpha1.ComponentSpec{
							Image: fmt.Sprintf("pingcap/tikv:%s", utilimage.TiDBVLatestPrev),
						},
					...
					PD: &v1alpha1.PDSpec{
						Replicas: 1,
						ComponentSpec: v1alpha1.ComponentSpec{
							Image: fmt.Sprintf("pingcap/pd:%s", utilimage.TiDBVLatestPrev),
						},
						ResourceRequirements: v1.ResourceRequirements{
							Requests: v1.ResourceList{
								v1.ResourceStorage: resource.MustParse("10G"),
			...
			err = controller.GuaranteedUpdate(genericCli, legacyTc, func() error {
				legacyTc.Spec.TiDB.Image = fmt.Sprintf("pingcap/tidb:%s", utilimage.TiDBVLatest)
				return nil
			})
			...
			err = controller.GuaranteedUpdate(genericCli, legacyTc, func() error {
				...
				legacyTc.Spec.PD.Version = pointer.StringPtr(utilimage.TiDBVLatest)
				return nil
			})
			...
			newTC := &v1alpha1.TidbCluster{
				...
				Spec: v1alpha1.TidbClusterSpec{
					TiDB: &v1alpha1.TiDBSpec{
						ComponentSpec: v1alpha1.ComponentSpec{
							Image: fmt.Sprintf("pingcap/tidb:%s", utilimage.TiDBVLatest),
						},
					},
					TiKV: &v1alpha1.TiKVSpec{
						ComponentSpec: v1alpha1.ComponentSpec{
							Image: fmt.Sprintf("pingcap/tikv:%s", utilimage.TiDBVLatest),
						},
						ResourceRequirements: v1.ResourceRequirements{
							Requests: v1.ResourceList{
								v1.ResourceStorage: resource.MustParse("10G"),
					...
					PD: &v1alpha1.PDSpec{
						ComponentSpec: v1alpha1.ComponentSpec{
							Image: fmt.Sprintf("pingcap/pd:%s", utilimage.TiDBVLatest),
						},
						ResourceRequirements: v1.ResourceRequirements{
							Requests: v1.ResourceList{
								v1.ResourceStorage: resource.MustParse("10G"),
	...
	ginkgo.Describe("Upgrading Operator from 1.1.7 to latest", func() {
		...
		ginkgo.It("should not change old TidbCluster", func() {
			ginkgo.By(fmt.Sprintf("deploy original tc %q", utilimage.TiDBVLatest))
			tcName := "tidbcluster"
	...
	ginkgo.Describe("Canary Deploy TiDB Operator", func() {
		...
		ginkgo.It("Deploy TidbCluster and check the result", func() {
			ginkgo.By(fmt.Sprintf("deploy original tc %q", utilimage.TiDBVLatestPrev))
			tcName := "tidbcluster1"
			...
			framework.ExpectNoError(err, "failed to update TidbCluster 1 to upgrade PD version to %v", utilimage.TiDBVLatest)
			...
			tc2 := fixture.GetTidbCluster(ns, tc2Name, utilimage.TiDBVLatestPrev)
			...
			err = controller.GuaranteedUpdate(genericCli, tc2, func() error {
				tc2.Spec.Version = utilimage.TiDBVLatest
				return nil
			})
			framework.ExpectNoError(err, "failed to update TidbCluster 2 to upgrade tidb version to %v", utilimage.TiDBVLatest)
			...
			ginkgo.By(fmt.Sprintf("wait for TidbCluster 2 pd-0 pod upgrading to %q", utilimage.TiDBVLatest))
			err = wait.Poll(5*time.Second, 10*time.Minute, func() (done bool, err error) {
				pdPod, err := c.CoreV1().Pods(ns).Get(fmt.Sprintf("%s-pd-0", tc2.Name), metav1.GetOptions{})
				if err != nil {
					return false, nil
				}
				if pdPod.Spec.Containers[0].Image != fmt.Sprintf("pingcap/pd:%s", utilimage.TiDBVLatest) {
					return false, nil
				}
				return true, nil
			})
			framework.ExpectNoError(err, "failed to upgrade TidbCluster 2 pd-0 to %q", utilimage.TiDBVLatest)
			log.Logf("Finished upgrading TidbCluster 2")
