...
var _ = ginkgo.Describe("Scale TiProxy", label.TiProxy, label.P0, label.Scale, func() {
	f := framework.New()
	f.Setup()
	...
	ginkgo.It("support scale from 3 to 5 and rolling restart at same time with maxSurge 2", label.Update, func(ctx context.Context) {
		...
		proxyg := action.MustCreateTiProxy(ctx, f, o,
			data.WithReplicas[scope.TiProxyGroup](3),
			data.WithTiProxyMaxSurge(2),
		)
		...
		action.MustScaleAndRollingRestart[scope.TiProxyGroup](ctx, f, proxyg, 5)
