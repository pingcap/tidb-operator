...
func SetupServerCert(namespaceName, serviceName string) (*CertContext, error) {
	...
	if err != nil {
		k8se2elog.Logf("Failed to create a temp dir for cert generation %v", err)
		return nil, err
	}
	defer os.RemoveAll(certDir)
	signingKey, err := pkiutil.NewPrivateKey()
	if err != nil {
		k8se2elog.Logf("Failed to create CA private key %v", err)
		return nil, err
	}
	signingCert, err := cert.NewSelfSignedCACert(cert.Config{CommonName: "e2e-server-cert-ca"}, signingKey)
	if err != nil {
		k8se2elog.Logf("Failed to create CA cert for apiserver %v", err)
		return nil, err
	}
	caCertFile, err := ioutil.TempFile(certDir, "ca.crt")
	if err != nil {
		k8se2elog.Logf("Failed to create a temp file for ca cert generation %v", err)
		return nil, err
	}
	if err := ioutil.WriteFile(caCertFile.Name(), pkiutil.EncodeCertPEM(signingCert), 0644); err != nil {
		k8se2elog.Logf("Failed to write CA cert %v", err)
		return nil, err
	}
	key, err := pkiutil.NewPrivateKey()
	if err != nil {
		k8se2elog.Logf("Failed to create private key for %v", err)
		return nil, err
	}
	...
	if err != nil {
		k8se2elog.Logf("Failed to create cert%v", err)
		return nil, err
	}
	certFile, err := ioutil.TempFile(certDir, "server.crt")
	if err != nil {
		k8se2elog.Logf("Failed to create a temp file for cert generation %v", err)
		return nil, err
	}
	keyFile, err := ioutil.TempFile(certDir, "server.key")
	if err != nil {
		k8se2elog.Logf("Failed to create a temp file for key generation %v", err)
		return nil, err
	}
	if err = ioutil.WriteFile(certFile.Name(), pkiutil.EncodeCertPEM(signedCert), 0600); err != nil {
		k8se2elog.Logf("Failed to write cert file %v", err)
		return nil, err
	}
	...
	if err = ioutil.WriteFile(keyFile.Name(), keyPEM, 0644); err != nil {
		k8se2elog.Logf("Failed to write key file %v", err)
		return nil, err
	}
	return &CertContext{
