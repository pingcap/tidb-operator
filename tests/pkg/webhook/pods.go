...
func (wh *webhook) admitPods(ar v1beta1.AdmissionReview) *v1beta1.AdmissionResponse {
	...
	if !wh.namespaces.Has(namespace) {
		k8se2elog.Logf("%q is not in our namespaces %v, skip", namespace, wh.namespaces.List())
	...
	if err != nil {
		k8se2elog.Logf("api server send wrong pod info namespace %s name %s err %v", namespace, name, err)
		return &reviewResponse
	}

	k8se2elog.Logf("delete %s pod [%s]", pod.Labels[label.ComponentLabelKey], pod.GetName())

	tc, err := versionCli.PingcapV1alpha1().TidbClusters(namespace).Get(pod.Labels[label.InstanceLabelKey], metav1.GetOptions{})
	if err != nil {
		k8se2elog.Logf("fail to fetch tidbcluster info namespace %s clustername(instance) %s err %v", namespace, pod.Labels[label.InstanceLabelKey], err)
		return &reviewResponse
	}
	...
	if pod.DeletionTimestamp != nil {
		k8se2elog.Logf("pod:[%s/%s] status is timestamp %s", namespace, name, pod.DeletionTimestamp)
		reviewResponse.Allowed = true
		return &reviewResponse
	}
	...
	if pod.Labels[label.ComponentLabelKey] == "pd" {
		...
		k8se2elog.Logf("savely delete pod namespace %s name %s leader name %s", namespace, name, leader.Name)
	}
	reviewResponse.Allowed = true
	return &reviewResponse
}
