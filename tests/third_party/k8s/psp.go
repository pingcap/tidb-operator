...
import (
	...
	e2eauth "github.com/pingcap/tidb-operator/tests/third_party/k8s/auth"
...
func IsPodSecurityPolicyEnabled(kubeClient clientset.Interface) bool {
	isPSPEnabledOnce.Do(func() {
		...
		log.Logf("Found PodSecurityPolicies; testing pod creation to see if PodSecurityPolicy is enabled")
		testPod := &v1.Pod{
		...
		if err != nil {
			if strings.Contains(err.Error(), "PodSecurityPolicy") {
				log.Logf("PodSecurityPolicy error creating dryrun pod; assuming PodSecurityPolicy is enabled: %v", err)
				isPSPEnabled = true
			} else {
				log.Logf("Error creating dryrun pod; assuming PodSecurityPolicy is disabled: %v", err)
			}
			return
		}
		pspAnnotation, pspAnnotationExists := dryRunPod.Annotations["kubernetes.io/psp"]
		if !pspAnnotationExists {
			log.Logf("No PSP annotation exists on dry run pod; assuming PodSecurityPolicy is disabled")
			return
		}
		log.Logf("PSP annotation exists on dry run pod: %q; assuming PodSecurityPolicy is enabled", pspAnnotation)
		isPSPEnabled = true
	})
	return isPSPEnabled
}
