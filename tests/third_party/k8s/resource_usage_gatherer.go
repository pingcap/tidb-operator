...
// this file is copied (and modified) from k8s.io/kubernetes/test/e2e/framework/resource_usage_gatherer.go @v1.23.17
...
func nodeHasControlPlanePods(c clientset.Interface, nodeName string) (bool, error) {
	...
	if len(podList.Items) < 1 {
		log.Logf("Can't find any pods in namespace %s to grab metrics from", metav1.NamespaceSystem)
	}
...
func getKubemarkMasterComponentsResourceUsage() map[string]*kubemarkResourceUsage {
	...
	if err != nil {
		log.Logf("Error when trying to SSH to master machine. Skipping probe. %v", err)
		return nil
	}
	...
	if err != nil {
		log.Logf("Error when trying to SSH to master machine. Skipping probe")
		return nil
	}
	scanner = bufio.NewScanner(strings.NewReader(sshResult))
	for scanner.Scan() {
