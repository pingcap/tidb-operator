...
func (oa *operatorActions) runKubectlOrDie(args ...string) string {
	cmd := "kubectl"
	k8se2elog.Logf("Running '%s %s'", cmd, strings.Join(args, " "))
	out, err := exec.Command(cmd, args...).CombinedOutput()
	if err != nil {
		k8se2elog.Failf("Failed to run '%s %s'\nCombined output: %q\nError: %v", cmd, strings.Join(args, " "), string(out), err)
	}
	k8se2elog.Logf("Combined output: %q", string(out))
	return string(out)
}
...
func (oa *operatorActions) InstallCRDOrDie(info *OperatorConfig) {
	...
	k8se2elog.Logf("Wait for all CRDs are established")
...
func (oa *operatorActions) DeployOperator(info *OperatorConfig) error {
	...
	k8se2elog.Logf("deploy operator response: %v\n", string(res))

	k8se2elog.Logf("Wait for all apiesrvices are available")
...
func (oa *operatorActions) CleanOperator(info *OperatorConfig) error {
	k8se2elog.Logf("cleaning tidb-operator %s", info.ReleaseName)
...
func (oa *operatorActions) UpgradeOperator(info *OperatorConfig) error {
	k8se2elog.Logf("upgrading tidb-operator %s", info.ReleaseName)
...
func ensurePodsUnchanged(pods1, pods2 *corev1.PodList) error {
	...
	if reflect.DeepEqual(pods1UIDs, pods2UIDs) {
		k8se2elog.Logf("%s", string(pods1Yaml))
		k8se2elog.Logf("%s", string(pods2Yaml))
		k8se2elog.Logf("%v", pods1UIDs)
		k8se2elog.Logf("%v", pods2UIDs)
		k8se2elog.Logf("pods unchanged after operator upgraded")
		return nil
	}
...
func (oa *operatorActions) DeployTidbCluster(info *TidbClusterConfig) error {
	...
	k8se2elog.Logf("deploying tidb cluster [%s/%s]", info.Namespace, info.ClusterName)
...
func (oa *operatorActions) CleanTidbCluster(info *TidbClusterConfig) error {
	k8se2elog.Logf("cleaning tidbcluster %s/%s", info.Namespace, info.ClusterName)
	...
	k8se2elog.Logf("%v", beforePVCNames)
	...
	for _, pv := range pvList.Items {
		...
		k8se2elog.Logf("%s, %s, %v", pv.Name, pv.Spec.PersistentVolumeReclaimPolicy, pv.Labels)
		k8se2elog.Logf("%v", pv.Spec.ClaimRef)
	}
	k8se2elog.Logf("%v", beforePVNames)
	...
	k8se2elog.Logf("%v", afterPVCNames)
	...
	waitPVFn := func() (done bool, err error) {
		...
		k8se2elog.Logf("%v", afterPVNames)

		if !reflect.DeepEqual(beforePVNames, afterPVNames) {
			k8se2elog.Logf("ERROR: pv changed when we delete cluster: %s/%s, before: %v, after: %v",
	...
	err = wait.Poll(10*time.Second, 5*time.Minute, func() (done bool, err error) {
		...
		k8se2elog.Logf(patchPVCmd)
		if res, err := exec.Command("/bin/sh", "-c", patchPVCmd).CombinedOutput(); err != nil {
			k8se2elog.Logf("ERROR: failed to patch pv: %v, %s", err, string(res))
			return false, nil
		}
		return true, nil
	})
	...
	pollFn := func() (bool, error) {
		if res, err := exec.Command("kubectl", "get", "po", "--output=name", "-n", info.Namespace, "-l", setStr).
			...
			k8se2elog.Logf("waiting for tidbcluster: %s/%s pods deleting, %v, [%s]",
				info.Namespace, info.ClusterName, err, string(res))
			return false, nil
		}
		...
		k8se2elog.Logf(pvCmd)
		if res, err := exec.Command("/bin/sh", "-c", pvCmd).CombinedOutput(); len(res) == 0 {
			return true, nil
		} else if err != nil {
			k8se2elog.Logf("waiting for tidbcluster: %s/%s pv deleting, %v, %s",
				info.Namespace, info.ClusterName, err, string(res))
			return false, nil
		}
		return true, nil
...
func (oa *operatorActions) CheckTidbClusterStatus(info *TidbClusterConfig) error {
	k8se2elog.Logf("checking tidb cluster [%s/%s] status", info.Namespace, info.ClusterName)
	...
	if err := wait.Poll(oa.pollInterval, 120*time.Minute, func() (bool, error) {
		...
		if tc, err = oa.cli.PingcapV1alpha1().TidbClusters(ns).Get(tcName, metav1.GetOptions{}); err != nil {
			k8se2elog.Logf("ERROR: failed to get tidbcluster: %s/%s, %v", ns, tcName, err)
			return false, nil
		}
		...
		k8se2elog.Logf("check tidb cluster begin tidbMembersReadyFn")
		if b, err := oa.tidbMembersReadyFn(tc); !b && err == nil {
			return false, nil
		}

		k8se2elog.Logf("check tidb cluster begin reclaimPolicySyncFn")
		if b, err := oa.reclaimPolicySyncFn(tc); !b && err == nil {
			return false, nil
		}

		k8se2elog.Logf("check tidb cluster begin metaSyncFn")
		...
		k8se2elog.Logf("check tidb cluster begin schedulerHAFn")
		if b, err := oa.schedulerHAFn(tc); !b && err == nil {
			return false, nil
		}

		k8se2elog.Logf("check all pd and tikv instances have not pod scheduling annotation")
		...
		k8se2elog.Logf("check store labels")
		...
		k8se2elog.Logf("check tidb cluster begin passwordIsSet")
		if b, err := oa.passwordIsSet(info); !b && err == nil {
			return false, nil
		}

		if info.Monitor {
			k8se2elog.Logf("check tidb monitor normal")
			if b, err := oa.monitorNormal(info); !b && err == nil {
				return false, nil
			}
		}
		if info.EnableConfigMapRollout {
			k8se2elog.Logf("check tidb cluster configuration synced")
			if b, err := oa.checkTidbClusterConfigUpdated(tc, info); !b && err == nil {
				return false, nil
			}
		}
		if info.EnablePVReclaim {
			k8se2elog.Logf("check reclaim pvs success when scale in pd or tikv")
		...
		k8se2elog.Logf("ERROR: check tidb cluster status failed: %s", err.Error())
...
func (oa *operatorActions) BeginInsertDataTo(info *TidbClusterConfig) error {
	...
	k8se2elog.Logf("begin insert Data in pod[%s/%s]", pod.Namespace, pod.Name)
	return nil
}
...
func (oa *operatorActions) CheckScaleInSafely(info *TidbClusterConfig) error {
	return wait.Poll(oa.pollInterval, DefaultPollTimeout, func() (done bool, err error) {
		...
		if err != nil {
			k8se2elog.Logf("ERROR: failed to get tidbcluster when scale in tidbcluster, error: %v", err)
			return false, nil
		}
		...
		if err != nil {
			k8se2elog.Logf("ERROR: failed to get tikvSet statefulset: [%s], error: %v", tikvSetName, err)
			return false, nil
		}

		pdClient, cancel, err := oa.getPDClient(tc)
		if err != nil {
			k8se2elog.Logf("ERROR: failed to create external PD client for tidb cluster %q: %v", tc.GetName(), err)
			return false, nil
		}
		defer cancel()
		...
		if err != nil {
			k8se2elog.Logf("pdClient.GetStores failed,error: %v", err)
			return false, nil
		}
		if len(stores.Stores) > int(*tikvSet.Spec.Replicas) {
			k8se2elog.Logf("stores.Stores: %v", stores.Stores)
			k8se2elog.Logf("tikvSet.Spec.Replicas: %d", *tikvSet.Spec.Replicas)
...
func (oa *operatorActions) CheckScaledCorrectly(info *TidbClusterConfig, podUIDsBeforeScale map[string]types.UID) error {
	return wait.Poll(oa.pollInterval, DefaultPollTimeout, func() (done bool, err error) {
		...
		if err != nil {
			k8se2elog.Logf("ERROR: failed to get pd pods's uid, error: %v", err)
			return false, nil
		}
...
func (oa *operatorActions) CheckUpgrade(ctx context.Context, info *TidbClusterConfig) error {
	...
	for i := replicas - 1; i >= 0; i-- {
		err := wait.PollImmediate(1*time.Second, 10*time.Minute, func() (done bool, err error) {
			...
			if err != nil {
				k8se2elog.Logf("ERROR: failed to get evict leader schedulers, %v", err)
				return false, nil
			}
			k8se2elog.Logf("index:%d,schedulers:%v,error:%v", i, schedulers, err)
			if len(schedulers) > 1 {
				k8se2elog.Logf("ERROR: there are too many evict leader schedulers: %v", schedulers)
				for _, s := range schedulers {
					if s == scheduler {
						k8se2elog.Logf("found scheudler: %s", scheduler)
						return true, nil
					}
				}
				return false, nil
			...
			if schedulers[0] == scheduler {
				k8se2elog.Logf("index: %d,the schedulers: %s = %s", i, schedulers[0], scheduler)
				return true, nil
			}
			k8se2elog.Logf("ERROR: index: %d, the scheduler: %s != %s", i, schedulers[0], scheduler)
			return false, nil
		})
		if err != nil {
			k8se2elog.Logf("ERROR: failed to check upgrade %s/%s, %v", ns, tcName, err)
			return err
		}
	}
	...
	return wait.PollImmediate(1*time.Second, 6*time.Minute, func() (done bool, err error) {
		...
		k8se2elog.Logf("ERROR: schedulers: %v is not empty", schedulers)
		return false, nil
	})
}
...
func getMemberContainer(kubeCli kubernetes.Interface, stsGetter typedappsv1.StatefulSetsGetter, namespace, tcName, component string) (*corev1.Container, bool) {
	...
	if err != nil {
		k8se2elog.Logf("ERROR: failed to get sts for component %s of cluster %s/%s", component, namespace, tcName)
		return nil, false
	}
...
func getStsContainer(kubeCli kubernetes.Interface, sts *apps.StatefulSet, containerName string) (*corev1.Container, bool) {
	...
	if err != nil {
		k8se2elog.Logf("ERROR: fail to get pods for container %s of sts %s/%s", containerName, sts.Namespace, sts.Name)
		return nil, false
	}
	if len(podList.Items) == 0 {
		k8se2elog.Logf("ERROR: no pods found for component %s of cluster %s/%s", containerName, sts.Namespace, sts.Name)
		return nil, false
	}
	pod := podList.Items[0]
	if len(pod.Spec.Containers) == 0 {
		k8se2elog.Logf("ERROR: no containers found for component %s of cluster %s/%s", containerName, sts.Namespace, sts.Name)
		return nil, false
	}
...
func (oa *operatorActions) pdMembersReadyFn(tc *v1alpha1.TidbCluster) (bool, error) {
	...
	if tc.Status.PD.StatefulSet == nil {
		k8se2elog.Logf("tidbcluster: %s/%s .status.PD.StatefulSet is nil", ns, tcName)
		return false, nil
	}
	...
	if !found {
		k8se2elog.Logf("statefulset: %s/%s not found containers[name=pd] or pod %s-0",
			ns, pdSetName, pdSetName)
		return false, nil
	}

	if tc.PDImage() != c.Image {
		k8se2elog.Logf("statefulset: %s/%s .spec.template.spec.containers[name=pd].image(%s) != %s",
	...
	for _, member := range tc.Status.PD.Members {
		if !member.Health {
			k8se2elog.Logf("tidbcluster: %s/%s pd member(%s/%s) is not health",
...
func (oa *operatorActions) tikvMembersReadyFn(tc *v1alpha1.TidbCluster) (bool, error) {
	...
	if tc.Status.TiKV.StatefulSet == nil {
		k8se2elog.Logf("tidbcluster: %s/%s .status.TiKV.StatefulSet is nil", ns, tcName)
		return false, nil
	}
	...
	if len(tc.Status.TiKV.Stores) != int(replicas) {
		k8se2elog.Logf("tidbcluster: %s/%s .status.TiKV.Stores.count(%d) != %d",
	...
	if !found {
		k8se2elog.Logf("statefulset: %s/%s not found containers[name=tikv] or pod %s-0",
			ns, tikvSetName, tikvSetName)
		return false, nil
	}

	if tc.TiKVImage() != c.Image {
		k8se2elog.Logf("statefulset: %s/%s .spec.template.spec.containers[name=tikv].image(%s) != %s",
	...
	for _, store := range tc.Status.TiKV.Stores {
		if store.State != v1alpha1.TiKVStateUp {
			k8se2elog.Logf("tidbcluster: %s/%s's store(%s) state != %s", ns, tcName, store.ID, v1alpha1.TiKVStateUp)
			return false, nil
		}
	}
...
func (oa *operatorActions) tiflashMembersReadyFn(tc *v1alpha1.TidbCluster) (bool, error) {
	...
	if err != nil {
		k8se2elog.Logf("ERROR: TiFlash failed to get statefulset: %s/%s, %v", ns, tiflashSetName, err)
		return false, nil
	}
	...
	if tc.Status.TiFlash.StatefulSet == nil {
		k8se2elog.Logf("tidbcluster: %s/%s .status.TiFlash.StatefulSet is nil", ns, tcName)
		return false, nil
	}
	...
	if !found {
		k8se2elog.Logf("statefulset: %s/%s not found containers[name=tiflash] or pod %s-0",
			ns, tiflashSetName, tiflashSetName)
		return false, nil
	}

	if tc.TiFlashImage() != c.Image {
		k8se2elog.Logf("statefulset: %s/%s .spec.template.spec.containers[name=tiflash].image(%s) != %s",
	...
	for _, store := range tc.Status.TiFlash.Stores {
		if store.State != v1alpha1.TiKVStateUp {
			k8se2elog.Logf("TiFlash tidbcluster: %s/%s's store(%s) state != %s", ns, tcName, store.ID, v1alpha1.TiKVStateUp)
			return false, nil
		}
	}
	...
	k8se2elog.Logf("TiFlash ready: %s/%s", ns, tcName)
	return true, nil
}
...
func (oa *operatorActions) tidbMembersReadyFn(tc *v1alpha1.TidbCluster) (bool, error) {
	...
	if tc.Status.TiDB.StatefulSet == nil {
		k8se2elog.Logf("tidbcluster: %s/%s .status.TiDB.StatefulSet is nil", ns, tcName)
		return false, nil
	}
	...
	if len(tc.Status.TiDB.Members) != int(tc.Spec.TiDB.Replicas) {
		k8se2elog.Logf("tidbcluster: %s/%s .status.TiDB.Members count(%d) != %d",
	...
	if !found {
		k8se2elog.Logf("statefulset: %s/%s not found containers[name=tidb] or pod %s-0",
			ns, tidbSetName, tidbSetName)
		return false, nil
	}

	if tc.TiDBImage() != c.Image {
		k8se2elog.Logf("statefulset: %s/%s .spec.template.spec.containers[name=tidb].image(%s) != %s",
...
func (oa *operatorActions) reclaimPolicySyncFn(tc *v1alpha1.TidbCluster) (bool, error) {
	...
	if pvcList, err = oa.kubeCli.CoreV1().PersistentVolumeClaims(ns).List(listOptions); err != nil {
		k8se2elog.Logf("ERROR: failed to list pvs for tidbcluster %s/%s, %v", ns, tcName, err)
		return false, nil
	}
	...
	for _, pvc := range pvcList.Items {
		...
		if pv, err := oa.kubeCli.CoreV1().PersistentVolumes().Get(pvName, metav1.GetOptions{}); err != nil {
			k8se2elog.Logf("ERROR: failed to get pv: %s, error: %v", pvName, err)
			return false, nil
		} else if pv.Spec.PersistentVolumeReclaimPolicy != *tc.Spec.PVReclaimPolicy {
			k8se2elog.Logf("ERROR: pv: %s's reclaimPolicy is not Retain", pvName)
			return false, nil
		}
	}
...
func (oa *operatorActions) metaSyncFn(tc *v1alpha1.TidbCluster) (bool, error) {
	...
	if cluster, err = pdClient.GetCluster(); err != nil {
		k8se2elog.Logf("ERROR: failed to get cluster from pdControl: %s/%s, error: %v", ns, tcName, err)
		return false, nil
	}
	...
	if podList, err = oa.kubeCli.CoreV1().Pods(ns).List(listOptions); err != nil {
		k8se2elog.Logf("ERROR: failed to list pods for tidbcluster %s/%s, %v", ns, tcName, err)
		return false, nil
	}
...
outerLoop:
	for _, pod := range podList.Items {
		...
		if pod.Labels[label.ClusterIDLabelKey] != clusterID {
			k8se2elog.Logf("tidbcluster %s/%s's pod %s's label %s not equals %s ",
		...
		case label.PDLabelVal:
			...
			if err != nil {
				k8se2elog.Logf("ERROR: failed to get members for tidbcluster %s/%s, %v", ns, tcName, err)
				return false, nil
			}
			...
			if memberID == "" {
				k8se2elog.Logf("tidbcluster: %s/%s's pod %s label [%s] is empty",
		...
		case label.TiKVLabelVal:
			...
			stores, err := pdClient.GetStores()
			if err != nil {
				k8se2elog.Logf("ERROR: failed to get stores for tidbcluster %s/%s, %v", ns, tcName, err)
				return false, nil
			}
		...
		if pvc, err = oa.kubeCli.CoreV1().PersistentVolumeClaims(ns).Get(pvcName, metav1.GetOptions{}); err != nil {
			k8se2elog.Logf("ERROR: failed to get pvc %s/%s for pod %s/%s", ns, pvcName, ns, podName)
			return false, nil
		}
		...
		if pv, err = oa.kubeCli.CoreV1().PersistentVolumes().Get(pvName, metav1.GetOptions{}); err != nil {
			k8se2elog.Logf("ERROR: failed to get pv for pvc %s/%s, %v", ns, pvcName, err)
			return false, nil
		}
...
func (oa *operatorActions) podsScheduleAnnHaveDeleted(tc *v1alpha1.TidbCluster) (bool, error) {
	...
	if err != nil {
		k8se2elog.Logf("ERROR: failed to list pvcs for tidb cluster %s/%s, err: %v", ns, tcName, err)
		return false, nil
	}
	...
	for _, pvc := range pvcList.Items {
		...
		if _, exist := pvc.Annotations[label.AnnPVCPodScheduling]; exist {
			k8se2elog.Logf("tidb cluster %s/%s pvc %s has pod scheduling annotation", ns, tcName, pvcName)
			return false, nil
		}
	}
...
func (oa *operatorActions) passwordIsSet(clusterInfo *TidbClusterConfig) (bool, error) {
	...
	if job, err = oa.kubeCli.BatchV1().Jobs(ns).Get(jobName, metav1.GetOptions{}); err != nil {
		k8se2elog.Logf("ERROR: failed to get job %s/%s, %v", ns, jobName, err)
		return false, nil
	}
	if job.Status.Succeeded < 1 {
		k8se2elog.Logf("tidbcluster: %s/%s password setter job not finished", ns, tcName)
		return false, nil
	}
	...
	if err != nil {
		k8se2elog.Logf("ERROR: failed to get TiDB DSN: %v", err)
		return false, nil
	}
	defer cancel()
	if db, err = sql.Open("mysql", dsn); err != nil {
		k8se2elog.Logf("can't open connection to mysql: %s, %v", dsn, err)
		return false, nil
	}
	defer db.Close()
	if err := db.Ping(); err != nil {
		k8se2elog.Logf("can't connect to mysql: %s with password %s, %v", dsn, clusterInfo.Password, err)
		return false, nil
	}
...
func (oa *operatorActions) monitorNormal(clusterInfo *TidbClusterConfig) (bool, error) {
	...
	if err != nil {
		k8se2elog.Logf("get monitor deployment: [%s/%s] failed", ns, monitorDeploymentName)
		return false, nil
	}
	if monitorDeployment.Status.ReadyReplicas < 1 {
		k8se2elog.Logf("monitor ready replicas %d < 1", monitorDeployment.Status.ReadyReplicas)
		return false, nil
	}
	if err := oa.checkPrometheus(clusterInfo); err != nil {
		k8se2elog.Logf("check [%s/%s]'s prometheus data failed: %v", ns, monitorDeploymentName, err)
		return false, nil
	}

	if err := oa.checkGrafanaData(clusterInfo); err != nil {
		k8se2elog.Logf("check [%s/%s]'s grafana data failed: %v", ns, monitorDeploymentName, err)
		return false, nil
	}
	return true, nil
}
...
func (oa *operatorActions) checkPdConfigUpdated(tc *v1alpha1.TidbCluster, clusterInfo *TidbClusterConfig) bool {
	...
	if err != nil {
		k8se2elog.Logf("ERROR: failed to get PD configuraion from tidb cluster [%s/%s]", tc.Namespace, tc.Name)
		return false
	}
	if len(clusterInfo.PDLogLevel) > 0 && clusterInfo.PDLogLevel != config.Log.Level {
		k8se2elog.Logf("check [%s/%s] PD logLevel configuration updated failed: desired [%s], actual [%s] not equal",
			tc.Namespace,
			tc.Name,
	...
	//	k8se2elog.Logf("check [%s/%s] PD maxReplicas configuration updated failed: desired [%d], actual [%d] not equal",
	//		tc.Namespace,
	//		tc.Name,
...
func (oa *operatorActions) checkTiDBConfigUpdated(tc *v1alpha1.TidbCluster, clusterInfo *TidbClusterConfig) bool {
	...
	if err != nil {
		k8se2elog.Logf("ERROR: failed to get pod ordinals for tidb cluster %s/%s (member: %v)", tc.Namespace, tc.Name, v1alpha1.TiDBMemberType)
		return false
	}
	for i := range ordinals {
		config, err := oa.tidbControl.GetSettings(tc, int32(i))
		if err != nil {
			k8se2elog.Logf("ERROR: failed to get TiDB configuration from cluster [%s/%s], ordinal: %d, error: %v", tc.Namespace, tc.Name, i, err)
			return false
		}
		if clusterInfo.TiDBTokenLimit > 0 && uint(clusterInfo.TiDBTokenLimit) != config.TokenLimit {
			k8se2elog.Logf("check [%s/%s] TiDB instance [%d] configuration updated failed: desired [%d], actual [%d] not equal",
...
func getDatasourceID(addr string) (int, error) {
	...
	defer func() {
		err := resp.Body.Close()
		if err != nil {
			k8se2elog.Logf("close response failed, err: %v", err)
		}
	}()
...
func (oa *operatorActions) DeployAdHocBackup(info *TidbClusterConfig) error {
	...
	k8se2elog.Logf("begin to deploy adhoc backup cluster[%s] namespace[%s]", info.ClusterName, info.Namespace)
	...
	getTSFn := func() (bool, error) {
		...
		if oa.fw != nil {
			...
			if err != nil {
				k8se2elog.Logf("ERROR: failed to forward port %d for %s/%s", 4000, info.Namespace, info.ClusterName)
				return false, nil
			}
			defer cancel()
			mysqlHost = localHost
		...
		k8se2elog.Logf(getTSCmd)

		res, err := exec.Command("/bin/bash", "-c", getTSCmd).CombinedOutput()
		if err != nil {
			k8se2elog.Logf("ERROR: failed to get ts %v, %s", err, string(res))
			return false, nil
		}
		tsStr = string(res)
		return true, nil
	...
	k8se2elog.Logf("install adhoc deployment [%s]", cmd)
	res, err := exec.Command("/bin/sh", "-c", cmd).CombinedOutput()
	if err != nil {
...
func (oa *operatorActions) CheckAdHocBackup(info *TidbClusterConfig) (string, error) {
	k8se2elog.Logf("checking adhoc backup cluster[%s] namespace[%s]", info.ClusterName, info.Namespace)
	...
	fn := func() (bool, error) {
		...
		if err != nil {
			k8se2elog.Logf("ERROR: failed to get jobs %s ,%v", jobName, err)
			return false, nil
		}
		if job.Status.Succeeded == 0 {
			k8se2elog.Logf("cluster [%s] back up job is not completed, please wait! ", info.ClusterName)
			return false, nil
		}
		...
		if err != nil {
			k8se2elog.Logf("ERROR: failed to list pods: %v", err)
			return false, nil
		}
		...
		if podName == "" {
			k8se2elog.Logf("ERROR: failed to find the ad-hoc backup: %s podName", jobName)
			return false, nil
		}
		...
		if err != nil {
			k8se2elog.Logf("ERROR: failed to get ts of pod %s, %v", podName, err)
			return false, nil
		}
		if string(tsData) == "" {
			k8se2elog.Logf("ts is empty pod %s", podName)
			return false, nil
		}

		ts = strings.TrimSpace(string(tsData))
		k8se2elog.Logf("ad-hoc backup ts: %s", ts)
...
func (oa *operatorActions) Restore(from *TidbClusterConfig, to *TidbClusterConfig) error {
	...
	k8se2elog.Logf("deploying restore, the data is from cluster[%s/%s] to cluster[%s/%s]",
	...
	k8se2elog.Logf("install restore [%s]", cmd)
	res, err := exec.Command("/bin/sh", "-c", cmd).CombinedOutput()
	if err != nil {
...
func (oa *operatorActions) CheckRestore(from *TidbClusterConfig, to *TidbClusterConfig) error {
	k8se2elog.Logf("begin to check restore backup cluster[%s] namespace[%s]", from.ClusterName, from.Namespace)
	...
	fn := func() (bool, error) {
		...
		if job.Status.Succeeded == 0 {
			k8se2elog.Logf("cluster [%s] restore job is not completed, please wait! ", to.ClusterName)
			return false, nil
		}
		...
		if err != nil {
			...
			k8se2elog.Logf("check restore: %v", err)
		}
...
func (oa *operatorActions) DataIsTheSameAs(tc, otherInfo *TidbClusterConfig) (bool, error) {
	...
	for i := 0; i < tableNum; i++ {
		...
		k8se2elog.Logf("cluster %s/%s's table %s count(*) = %d and cluster %s/%s's table %s count(*) = %d",
			tc.Namespace, tc.ClusterName, tableName, cnt,
			otherInfo.Namespace, otherInfo.ClusterName, tableName, otherCnt)
	}
...
func (oa *operatorActions) DeployScheduledBackup(info *TidbClusterConfig) error {
	...
	k8se2elog.Logf("begin to deploy scheduled backup")
	...
	k8se2elog.Logf("scheduled-backup deploy [%s]", cmd)
	res, err := exec.Command("/bin/sh", "-c", cmd).CombinedOutput()
	if err != nil {
...
func (oa *operatorActions) disableScheduledBackup(info *TidbClusterConfig) error {
	k8se2elog.Logf("disabling scheduled backup")
	...
	k8se2elog.Logf("scheduled-backup disable [%s]", cmd)
	res, err := exec.Command("/bin/sh", "-c", cmd).CombinedOutput()
	if err != nil {
...
func (oa *operatorActions) CheckScheduledBackup(info *TidbClusterConfig) error {
	k8se2elog.Logf("checking scheduler backup for tidb cluster[%s/%s]", info.Namespace, info.ClusterName)
	...
	fn := func() (bool, error) {
		...
		if err != nil {
			k8se2elog.Logf("ERROR: failed to get cronjobs %s ,%v", jobName, err)
			return false, nil
		}

		jobs, err := oa.kubeCli.BatchV1().Jobs(info.Namespace).List(metav1.ListOptions{})
		if err != nil {
			k8se2elog.Logf("ERROR: failed to list jobs %s ,%v", info.Namespace, err)
			return false, nil
		}
		...
		if len(backupJobs) == 0 {
			k8se2elog.Logf("cluster [%s] scheduler jobs is creating, please wait!", info.ClusterName)
			return false, nil
		}
		...
		if succededJobCount >= 3 {
			k8se2elog.Logf("cluster [%s/%s] scheduled back up job completed count: %d",
				info.Namespace, info.ClusterName, succededJobCount)
			return true, nil
		}

		k8se2elog.Logf("cluster [%s/%s] scheduled back up job is not completed, please wait! ",
			info.Namespace, info.ClusterName)
		return false, nil
	}
...
func getParentUIDFromJob(j batchv1.Job) (types.UID, bool) {
	...
	if controllerRef.Kind != "CronJob" {
		k8se2elog.Logf("Job with non-CronJob parent, name %s namespace %s", j.Name, j.Namespace)
		return types.UID(""), false
	}
...
func (oa *operatorActions) getBackupDir(info *TidbClusterConfig) ([]string, error) {
	...
	if err != nil && !errors.IsAlreadyExists(err) {
		k8se2elog.Logf("cluster: [%s/%s] create get backup dir pod failed, error :%v", info.Namespace, info.ClusterName, err)
		return nil, err
	}
	...
	res, err := exec.Command("/bin/sh", "-c", cmd).CombinedOutput()
	if err != nil {
		k8se2elog.Logf("cluster:[%s/%s] exec :%s failed,error:%v,result:%s", info.Namespace, info.ClusterName, cmd, err, string(res))
		return nil, err
	}

	dirs := strings.Split(string(res), "\n")
	k8se2elog.Logf("dirs in pod info name [%s] dir name [%s]", scheduledPvcName, strings.Join(dirs, ","))
	return dirs, nil
}
...
func (oa *operatorActions) DeployIncrementalBackup(from *TidbClusterConfig, to *TidbClusterConfig, withDrainer bool, ts string) error {
	...
	if withDrainer {
		...
		k8se2elog.Logf("begin to deploy incremental backup, source cluster[%s/%s], target cluster [%s/%s]",
			from.Namespace, from.ClusterName, to.Namespace, to.ClusterName)
	} else {
		oa.EmitEvent(from, "Enable pump cluster")
		k8se2elog.Logf("begin to enable pump for cluster[%s/%s]",
...
func (oa *operatorActions) CheckIncrementalBackup(info *TidbClusterConfig, withDrainer bool) error {
	k8se2elog.Logf("begin to check incremental backup cluster[%s] namespace[%s]", info.ClusterName, info.Namespace)
	...
	fn := func() (bool, error) {
		...
		if pumpStatefulSet.Status.Replicas != pumpStatefulSet.Status.ReadyReplicas {
			k8se2elog.Logf("pump replicas is not ready, please wait ! %s ", pumpStatefulSetName)
			return false, nil
		}
		...
		for _, pod := range pods.Items {
			...
			k8se2elog.Logf("%v", pod.Spec.Affinity)
			...
			k8se2elog.Logf("%v", pod.Spec.Tolerations)
			foundKey := false
		...
		if drainerStatefulSet.Status.Replicas != drainerStatefulSet.Status.ReadyReplicas {
			k8se2elog.Logf("drainer replicas is not ready, please wait ! %s ", pumpStatefulSetName)
			return false, nil
		}
		...
		for _, pod := range pods.Items {
			if !oa.drainerHealth(info.ClusterName, info.Namespace, pod.Name, false) {
				k8se2elog.Logf("some pods is not health %s", drainerStatefulSetName)
				return false, nil
			}
...
func (oa *operatorActions) RegisterWebHookAndService(configName, namespace, service string, context *apimachinery.CertContext) error {
	client := oa.kubeCli
	k8se2elog.Logf("Registering the webhook via the AdmissionRegistration API")
	...
	if err != nil {
		k8se2elog.Logf("registering webhook config %s with namespace %s error %v", configName, namespace, err)
		return err
	}
...
func (oa *operatorActions) pumpHealth(tcName, ns, podName string, tlsEnabled bool) bool {
	...
	if oa.fw != nil {
		...
		if err != nil {
			k8se2elog.Logf("ERROR: failed to forward port %d for %s/%s", 8250, ns, podName)
			return false
		}
		defer cancel()
	...
	if err != nil {
		k8se2elog.Logf("cluster:[%s] call %s failed,error:%v", tcName, pumpHealthURL, err)
		return false
	}
	...
	if err != nil {
		k8se2elog.Logf("cluster:[%s] read response body failed,error:%v", tcName, err)
		return false
	}
	healths := pumpStatus{}
	err = json.Unmarshal(body, &healths)
	if err != nil {
		k8se2elog.Logf("cluster:[%s] unmarshal failed,error:%v", tcName, err)
		return false
	}
	for _, status := range healths.StatusMap {
		if status.State != "online" {
			k8se2elog.Logf("cluster:[%s] pump's state is not online", tcName)
			return false
		}
	}
	return true
...
func (oa *operatorActions) drainerHealth(tcName, ns, podName string, tlsEnabled bool) bool {
	...
	if oa.fw != nil {
		...
		if err != nil {
			k8se2elog.Logf("ERROR: failed to forward port %d for %s/%s", 8249, ns, podName)
			return false
		}
		defer cancel()
		addr = fmt.Sprintf("%s:%d", localHost, localPort)
	...
	if err != nil {
		k8se2elog.Logf("cluster:[%s] call %s failed,error:%v", tcName, drainerHealthURL, err)
		return false
	}
	if res.StatusCode >= 400 {
		k8se2elog.Logf("Error response %v", res.StatusCode)
...
func (oa *operatorActions) EmitEvent(info *TidbClusterConfig, message string) {
	...
	k8se2elog.Logf("Event: %s", message)
...
func (oa *operatorActions) RunEventWorker() {
	...
	oa.lock.Unlock()
	k8se2elog.Logf("Event worker started")
	...
	for key, clusterEv := range oa.clusterEvents {
		...
		for _, ev := range clusterEv.events {
			...
			if err := client.AddAnnotation(anno); err != nil {
				k8se2elog.Logf("cluster:[%s/%s] error recording event: %s, reason: %v",
					ns, clusterName, ev.message, err)
				retryEvents = append(retryEvents, ev)
				continue
			}
			k8se2elog.Logf("cluster: [%s/%s] recoding event: %s", ns, clusterName, ev.message)
		}
...
func (oa *operatorActions) checkManualPauseComponent(info *TidbClusterConfig, component string) error {
	...
	fn := func() (bool, error) {
		...
		if tc, err = oa.cli.PingcapV1alpha1().TidbClusters(ns).Get(info.ClusterName, metav1.GetOptions{}); err != nil {
			k8se2elog.Logf("ERROR: failed to get tidbcluster: [%s/%s], %v", ns, info.ClusterName, err)
			return false, nil
		}
		...
		case label.TiDBLabelVal:
			...
			if err != nil {
				k8se2elog.Logf("fail to get pod in CheckManualPauseCompoent tidb [%s/%s]", ns, podName)
				return false, nil
			}
			...
			if tidbPod.Labels[v1.ControllerRevisionHashLabelKey] == tc.Status.TiDB.StatefulSet.UpdateRevision &&
				...
				if member, ok := tc.Status.TiDB.Members[tidbPod.Name]; !ok || !member.Health {
					k8se2elog.Logf("wait for tidb pod [%s/%s] ready member health %t ok %t", ns, podName, member.Health, ok)
				} else {
					return true, nil
				}
			} else {
				k8se2elog.Logf("tidbset is not in upgrade phase or pod is not upgrade done [%s/%s]", ns, podName)
			}
		...
		case label.TiKVLabelVal:
			...
			if err != nil {
				k8se2elog.Logf("fail to get pod in CheckManualPauseCompoent tikv [%s/%s]", ns, podName)
				return false, nil
			}
			...
			if tikvPod.Labels[v1.ControllerRevisionHashLabelKey] == tc.Status.TiKV.StatefulSet.UpdateRevision &&
				...
				if tikvStore == nil || tikvStore.State != v1alpha1.TiKVStateUp {
					k8se2elog.Logf("wait for tikv pod [%s/%s] ready store state %s", ns, podName, tikvStore.State)
				} else {
					return true, nil
				}
			} else {
				k8se2elog.Logf("tikvset is not in upgrade phase or pod is not upgrade done [%s/%s]", ns, podName)
			}
...
func (oa *operatorActions) CheckUpgradeComplete(info *TidbClusterConfig) error {
	...
	if err := wait.PollImmediate(15*time.Second, 30*time.Minute, func() (done bool, err error) {
		tc, err := oa.cli.PingcapV1alpha1().TidbClusters(ns).Get(tcName, metav1.GetOptions{})
		if err != nil {
			k8se2elog.Logf("checkUpgradeComplete, [%s/%s] cannot get tidbcluster, %v", ns, tcName, err)
			return false, nil
		}
		if tc.Status.PD.Phase == v1alpha1.UpgradePhase {
			k8se2elog.Logf("checkUpgradeComplete, [%s/%s] PD is still upgrading", ns, tcName)
			return false, nil
		}
		if tc.Status.TiKV.Phase == v1alpha1.UpgradePhase {
			k8se2elog.Logf("checkUpgradeComplete, [%s/%s] TiKV is still upgrading", ns, tcName)
			return false, nil
		}
		if tc.Status.TiDB.Phase == v1alpha1.UpgradePhase {
			k8se2elog.Logf("checkUpgradeComplete, [%s/%s] TiDB is still upgrading", ns, tcName)
			return false, nil
		}
		return true, nil
	}); err != nil {
		k8se2elog.Logf("ERROR: failed to wait upgrade complete [%s/%s], %v", ns, tcName, err)
		return err
	}
	return nil
}
...
func (oa *operatorActions) CheckInitSQL(info *TidbClusterConfig) error {
	...
	if err := wait.PollImmediate(10*time.Second, DefaultPollTimeout, func() (done bool, err error) {
		...
		k8se2elog.Logf("ERROR: failed to check init sql complete [%s/%s], %v", ns, tcName, err)
		return err
	}
	return nil
}
...
func (oa *operatorActions) pumpMembersReadyFn(tc *v1alpha1.TidbCluster) (bool, error) {
	...
	if err != nil {
		k8se2elog.Logf("ERROR: failed to get statefulset: %s/%s, %v", ns, ssName, err)
		return false, nil
	}
	...
	return wait.PollImmediate(pollInterval, timeout, func() (bool, error) {
		...
		if tc.Spec.TiFlash != nil {
			if b, err := oa.tiflashMembersReadyFn(local); !b && err == nil {
				k8se2elog.Logf("tiflash  members not ready: %s/%s, %v", tc.Namespace, tc.Name, err)
				return false, nil
			}
			k8se2elog.Logf("tiflash  members ready: %s/%s, %v", tc.Namespace, tc.Name, err)
		}
		if tc.Spec.Pump != nil {
