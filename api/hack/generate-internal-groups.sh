...
Usage: $(basename "$0") <generators> <output-package> <internal-apis-package> <extensiona-apis-package> <groups-versions> ...
  <generators>        the generators comma separated to run (deepcopy,defaulter,conversion,client,openapi) or "all".
...
# go install ./"$(dirname "${0}")"/cmd/{defaulter-gen,conversion-gen,client-gen,deepcopy-gen,openapi-gen}
