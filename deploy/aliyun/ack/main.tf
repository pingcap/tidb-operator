...
resource "alicloud_ess_scaling_group" "workers" {
  ...
  vswitch_ids = split(
    ",",
    var.vpc_id != "" ? join(",", data.template_file.vswitch_id.*.rendered) : join(",", alicloud_vswitch.all.*.id),
  )
  min_size = lookup(
    var.worker_groups[count.index],
    "min_size",
    var.group_default["min_size"],
  )
  max_size = lookup(
    var.worker_groups[count.index],
    "max_size",
    var.group_default["max_size"],
  )
  default_cooldown = lookup(
    var.worker_groups[count.index],
    "default_cooldown",
    var.group_default["default_cooldown"],
  )
  multi_az_policy = lookup(
    var.worker_groups[count.index],
    "multi_az_policy",
    var.group_default["multi_az_policy"],
  )
...
resource "alicloud_ess_scaling_configuration" "workers" {
  ...
  image_id = lookup(
    var.worker_groups[count.index],
    "image_id",
    var.group_default["image_id"],
  )
  instance_type = lookup(
    var.worker_groups[count.index],
    "instance_type",
    var.group_default["instance_type"],
  )
  security_group_id = var.group_id != "" ? var.group_id : alicloud_security_group.group[0].id
  key_name          = alicloud_key_pair.default[0].key_name
  system_disk_category = lookup(
    var.worker_groups[count.index],
    "system_disk_category",
    var.group_default["system_disk_category"],
  )
  system_disk_size = lookup(
    var.worker_groups[count.index],
    "system_disk_size",
    var.group_default["system_disk_size"],
  )
  user_data = element(data.template_file.userdata.*.rendered, count.index)
  internet_charge_type = lookup(
    var.worker_groups[count.index],
    "internet_charge_type",
    var.group_default["internet_charge_type"],
  )
  internet_max_bandwidth_in = lookup(
    var.worker_groups[count.index],
    "internet_max_bandwidth_in",
    var.group_default["internet_max_bandwidth_in"],
  )
  internet_max_bandwidth_out = lookup(
    var.worker_groups[count.index],
    "internet_max_bandwidth_out",
    var.group_default["internet_max_bandwidth_out"],
  )
  ...
  tags = merge(
    {
      ...
      "k8s.io/cluster-autoscaler/${lookup(
        var.worker_groups[count.index],
        "autoscaling_enabled",
        var.group_default["autoscaling_enabled"],
      ) == 1 ? "enabled" : "disabled"}"                                      = "true"
