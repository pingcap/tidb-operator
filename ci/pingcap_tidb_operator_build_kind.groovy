...
def call(BUILD_BRANCH, CREDENTIALS_ID, CODECOV_CREDENTIALS_ID) {
	...
	catchError {
        ...
        def builds = [:]
        builds["E2E v1.12.10"] = {
			build("IMAGE_TAG=${GITHASH} SKIP_BUILD=y GINKGO_NODES=8 KUBE_VERSION=v1.12.10 DOCKER_IO_MIRROR=http://docker.mirrors.ustc.edu.cn make e2e")
        }
        builds["E2E v1.16.3"] = {
			build("IMAGE_TAG=${GITHASH} SKIP_BUILD=y GINKGO_NODES=8 KUBE_VERSION=v1.16.3 DOCKER_IO_MIRROR=http://docker.mirrors.ustc.edu.cn make e2e")
        }
