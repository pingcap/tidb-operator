...
func TestPVCNewer(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			obj: fake.FakeObj("aaa", func(obj *v1alpha1.TiKV) *v1alpha1.TiKV {
				return obj
			}),
	...
	for i := range cases {
		...
		t.Run(c.desc, func(tt *testing.T) {
			tt.Parallel()
			pvcs := coreutil.PVCs[scope.TiKV](c.c, c.obj, coreutil.WithLegacyK8sAppLabels(), coreutil.EnableVAC(c.enableVAC))
