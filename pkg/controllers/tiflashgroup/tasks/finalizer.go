		...
		for _, peer := range rtx.Peers {
			...
			if peer.GetDeletionTimestamp().IsZero() {
				if err := t.Client.Delete(ctx, peer); err != nil {
					errList = append(errList, fmt.Errorf("try to delete the tiflash instance %v failed: %w", peer.Name, err))
				}
			}
		}

		if len(errList) != 0 {
			return task.Fail().With("failed to delete all tiflash instances: %v", utilerr.NewAggregate(errList))
		}

		if len(rtx.Peers) != 0 {
			return task.Fail().With("wait for all tiflash instances being removed, %v still exists", names)
		}

		if err := k8s.EnsureGroupSubResourceDeleted(ctx, t.Client,
			rtx.TiFlashGroup.Namespace, rtx.TiFlashGroup.Name); err != nil {
			return task.Fail().With("cannot delete subresources: %w", err)
		}
		if err := k8s.RemoveFinalizer(ctx, t.Client, rtx.TiFlashGroup); err != nil {
			return task.Fail().With("failed to ensure finalizer has been removed: %w", err)
		}
	} else {
		if err := k8s.EnsureFinalizer(ctx, t.Client, rtx.TiFlashGroup); err != nil {
