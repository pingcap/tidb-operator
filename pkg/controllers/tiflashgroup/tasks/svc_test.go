...
func TestTaskService(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			objs: []client.Object{
				fake.FakeObj[corev1.Service]("aaa-tiflash-peer"),
			},
		...
		},
		{
			...
			objs: []client.Object{
				newHeadlessService(fake.FakeObj[v1alpha1.TiFlashGroup]("aaa")),
			},
			unexpectedErr: true,
	...
	for i := range cases {
		...
		t.Run(c.desc, func(tt *testing.T) {
			...
			fc := client.NewFakeClient(c.state.TiFlashGroup())
			...
			if !c.unexpectedErr {
				...
				assert.Len(tt, svcs.Items, 1, c.desc)
			}
		})
	}
}
