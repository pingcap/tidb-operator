		...
		UpdateFunc: func(ctx context.Context, event event.TypedUpdateEvent[client.Object],
			...
			if err := r.Client.List(ctx, &list, client.InNamespace(cluster.Namespace),
				...
				if !errors.IsNotFound(err) {
					r.Logger.Error(err, "cannot list all tiflash groups", "ns", cluster.Namespace, "cluster", cluster.Name)
				}
				return
			}

			for i := range list.Items {
				flashGroup := &list.Items[i]
				queue.Add(reconcile.Request{
					NamespacedName: types.NamespacedName{
						Name:      flashGroup.Name,
						Namespace: flashGroup.Namespace,
					},
				})
			}
		},
