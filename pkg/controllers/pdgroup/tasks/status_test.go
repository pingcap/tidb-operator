...
func TestTaskStatusSuspend(t *testing.T) {
	cases := []struct {
		desc          string
		state         State
		unexpectedErr bool
	...
	}{
		{
			...
			expectedObj: fake.FakeObj("aaa", fake.SetGeneration[v1alpha1.PDGroup](3), func(obj *v1alpha1.PDGroup) *v1alpha1.PDGroup {
				...
				obj.Status.Conditions = append(obj.Status.Conditions, metav1.Condition{
					...
					Message:            "pd group is suspended",
				})
				return obj
			}),
		},
		{
			desc: "all pds are suspended",
			state: &state{
				pdg: fake.FakeObj("aaa", fake.SetGeneration[v1alpha1.PDGroup](3)),
				pds: []*v1alpha1.PD{
		...
		},
		{
			...
			expectedObj: fake.FakeObj("aaa", fake.SetGeneration[v1alpha1.PDGroup](3), func(obj *v1alpha1.PDGroup) *v1alpha1.PDGroup {
				...
				obj.Status.Conditions = append(obj.Status.Conditions, metav1.Condition{
					...
					ObservedGeneration: 3,
					Reason:             v1alpha1.PDGroupSuspendReason,
					Message:            "pd group is suspending",
				})
				return obj
			}),
		},
		{
			desc: "all pds are suspended but cannot call api",
			state: &state{
				pdg: fake.FakeObj("aaa", fake.SetGeneration[v1alpha1.PDGroup](3)),
				pds: []*v1alpha1.PD{
					fake.FakeObj("aaa", func(obj *v1alpha1.PD) *v1alpha1.PD {
		...
		{
			desc: "all pds are suspended and pdg is up to date and cannot call api",
			state: &state{
	...
	for i := range cases {
		...
		t.Run(c.desc, func(tt *testing.T) {
			...
			res, done := task.RunTask(ctx, TaskStatusSuspend(c.state, fc))
