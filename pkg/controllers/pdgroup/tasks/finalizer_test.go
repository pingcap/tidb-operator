...
func TestTaskFinalizerAdd(t *testing.T) {
		...
		},
		{
			...
			state: &state{
				pdg: fake.FakeObj("aaa", func(obj *v1alpha1.PDGroup) *v1alpha1.PDGroup {
					obj.Finalizers = append(obj.Finalizers, "xxxx")
					return obj
				}),
			},
			expectedStatus: task.SComplete,
			expectedObj: fake.FakeObj("aaa", func(obj *v1alpha1.PDGroup) *v1alpha1.PDGroup {
				obj.Finalizers = append(obj.Finalizers, "xxxx", v1alpha1.Finalizer)
				return obj
			}),
		},
		{
	...
	for i := range cases {
		...
		t.Run(c.desc, func(tt *testing.T) {
			...
			res, done := task.RunTask(ctx, TaskFinalizerAdd(c.state, fc))
