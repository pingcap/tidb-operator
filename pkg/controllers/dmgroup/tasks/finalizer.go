...
func TaskFinalizerDel(state State, c client.Client) task.Task {
	return task.NameTaskFunc("FinalizerDel", func(ctx context.Context) task.Result {
		var errList []error
		for _, dm := range state.DMSlice() {
			if dm.GetDeletionTimestamp().IsZero() {
				if err := c.Delete(ctx, dm); err != nil {
		...
		if len(errList) != 0 {
			return task.Fail().With("failed to delete all dm-master instances: %v", utilerr.NewAggregate(errList))
		}

		if len(state.DMSlice()) != 0 {
			return task.Wait().With("wait for all dm-master instances being removed, %v still exists", len(state.DMSlice()))
		}

		wait, err := k8s.DeleteGroupSubresource(ctx, c, runtime.FromDMGroup(state.DMGroup()), &corev1.ServiceList{})
		if err != nil {
