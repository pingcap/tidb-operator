...
func TaskService(state common.ObjectState[*v1alpha1.ResourceManagerGroup], c client.Client) task.Task {
	return task.NameTaskFunc("Service", func(ctx context.Context) task.Result {
		...
		svc := newHeadlessService(rmg)
		if err := c.Apply(ctx, svc); err != nil {
			return task.Fail().With(fmt.Sprintf("can't create headless service of resource manager: %v", err))
		}

		return task.Complete().With("headless service of resource manager has been applied")
	})
}

func newHeadlessService(rmg *v1alpha1.ResourceManagerGroup) *corev1.Service {
	ipFamilyPolicy := corev1.IPFamilyPolicyPreferDualStack
	return &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      coreutil.HeadlessServiceName[scope.ResourceManagerGroup](rmg),
			...
			Ports: []corev1.ServicePort{
				{
					...
					Port:       coreutil.ResourceManagerGroupClientPort(rmg),
					Protocol:   corev1.ProtocolTCP,
					TargetPort: intstr.FromString(v1alpha1.ResourceManagerPortNameClient),
				},
			},
