...
func TestTaskStatus(t *testing.T) {
	...
	cases := []struct {
		...
		expectedObj    *v1alpha1.TiProxy
	}{
		{
			desc: "not ready",
			state: &ReconcileContext{
				State: &state{
					tiproxy: fake.FakeObj(fakeTiProxyName, func(obj *v1alpha1.TiProxy) *v1alpha1.TiProxy {
						obj.Generation = 3
			...
			expectedObj: fake.FakeObj(fakeTiProxyName, func(obj *v1alpha1.TiProxy) *v1alpha1.TiProxy {
				obj.Generation = 3
				obj.Labels = map[string]string{
					v1alpha1.LabelKeyInstanceRevisionHash: newRevision,
				}
	...
	for i := range cases {
		...
		t.Run(c.desc, func(tt *testing.T) {
			...
			objs = append(objs, c.state.TiProxy())
			if c.state.Pod() != nil {
			...
			require.NoError(tt, fc.Get(ctx, client.ObjectKey{Name: fakeTiProxyName}, obj), c.desc)
