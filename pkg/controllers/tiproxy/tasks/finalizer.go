...
func TaskFinalizerDel(state *ReconcileContext, c client.Client) task.Task {
	return task.NameTaskFunc("FinalizerDel", func(ctx context.Context) task.Result {
		wait, err := EnsureSubResourcesDeleted(ctx, c, state.TiProxy())
		if err != nil {
		...
		if err := k8s.RemoveFinalizer(ctx, c, state.TiProxy()); err != nil {
...
func EnsureSubResourcesDeleted(ctx context.Context, c client.Client, tiproxy *v1alpha1.TiProxy) (wait bool, _ error) {
	wait1, err := k8s.DeleteInstanceSubresource(ctx, c, runtime.FromTiProxy(tiproxy), &corev1.PodList{})
	if err != nil {
		return false, err
	}
	wait2, err := k8s.DeleteInstanceSubresource(ctx, c, runtime.FromTiProxy(tiproxy), &corev1.ConfigMapList{})
	if err != nil {
		return false, err
	}
	wait3, err := k8s.DeleteInstanceSubresource(ctx, c, runtime.FromTiProxy(tiproxy), &corev1.PersistentVolumeClaimList{})
	if err != nil {
		return false, err
	}
