...
func TestTaskPod(t *testing.T) {
	...
	for i := range cases {
		...
		t.Run(c.desc, func(tt *testing.T) {
			...
			objs = append(objs, c.state.TiProxy(), c.state.Cluster())
			if c.state.Pod() != nil {
			...
			if c.expectUpdatedPod {
				expectedPod := newPod(c.state.Cluster(), c.state.TiProxy())
...
func fakePod(c *v1alpha1.Cluster, tidb *v1alpha1.TiProxy) *corev1.Pod {
