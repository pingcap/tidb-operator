...
func TaskPVC(state *ReconcileContext, logger logr.Logger, c client.Client, vm volumes.ModifierFactory) task.Task {
	return task.NameTaskFunc("PVC", func(ctx context.Context) task.Result {
		ck := state.Cluster()
		pvcs := newPVCs(state.Cluster(), state.TiProxy())
...
func newPVCs(cluster *v1alpha1.Cluster, tidb *v1alpha1.TiProxy) []*corev1.PersistentVolumeClaim {
	...
	for i := range tidb.Spec.Volumes {
		...
		pvcs = append(pvcs, &corev1.PersistentVolumeClaim{
			ObjectMeta: metav1.ObjectMeta{
				Name:      PersistentVolumeClaimName(coreutil.PodName[scope.TiProxy](tidb), vol.Name),
				Namespace: tidb.Namespace,
				Labels: maputil.Merge(
					coreutil.PersistentVolumeClaimLabels[scope.TiProxy](tidb, vol.Name),
					// legacy labels in v1
					map[string]string{
				...
				OwnerReferences: []metav1.OwnerReference{
					*metav1.NewControllerRef(tidb, v1alpha1.SchemeGroupVersion.WithKind("TiProxy")),
				},
			},
	...
	if tidb.Spec.Overlay != nil {
		for _, o := range tidb.Spec.Overlay.PersistentVolumeClaims {
