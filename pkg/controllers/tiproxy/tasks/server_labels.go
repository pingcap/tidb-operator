...
var (
	// node labels that can be used as tiproxy DC label Name
	topologyZoneLabels = []string{"zone", corev1.LabelTopologyZone}

	// tiproxy DC label Name
	tiproxyDCLabel = "zone"
)
...
func TaskServerLabels(state *ReconcileContext, c client.Client) task.Task {
	...
	outer:
		...
		serverLabels[tiproxyDCLabel] = serverLabels[zoneLabel]

		// TODO: is there any way to avoid unnecessary update?
		if err := state.TiProxyClient.SetLabels(ctx, serverLabels); err != nil {
