...
func (r *Reconciler) ClusterEventHandler() handler.TypedEventHandler[client.Object, reconcile.Request] {
	return handler.TypedFuncs[client.Object, reconcile.Request]{
		UpdateFunc: func(ctx context.Context, event event.TypedUpdateEvent[client.Object],
			...
			var proxyList v1alpha1.TiProxyList
			if err := r.Client.List(ctx, &proxyList, client.MatchingLabels{
				...
				r.Logger.Error(err, "cannot list all tiproxy instances", "ns", newObj.Namespace, "cluster", newObj.Name)
				return
			}

			for i := range proxyList.Items {
				tiproxy := &proxyList.Items[i]
