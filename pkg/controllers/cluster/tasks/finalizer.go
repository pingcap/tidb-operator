...
func (t *TaskFinalizer) Sync(ctx task.Context[ReconcileContext]) task.Result {
	...
	for _, wg := range rtx.TiKVWorkerGroups {
		//nolint:gocritic // not a real issue, see https://github.com/go-critic/go-critic/issues/1448
		if err := t.Client.Delete(ctx, wg); client.IgnoreNotFound(err) != nil {
			return task.Fail().With("can't delete tikv worker group: %v", err)
		}
	}

	for _, dmg := range rtx.DMGroups {
		//nolint:gocritic // not a real issue, see https://github.com/go-critic/go-critic/issues/1448
		if err := t.Client.Delete(ctx, dmg); client.IgnoreNotFound(err) != nil {
			return task.Fail().With("can't delete dm group: %v", err)
		}
	}

	for _, dwg := range rtx.DMWorkerGroups {
		//nolint:gocritic // not a real issue, see https://github.com/go-critic/go-critic/issues/1448
		if err := t.Client.Delete(ctx, dwg); client.IgnoreNotFound(err) != nil {
			return task.Fail().With("can't delete dm worker group: %v", err)
		}
	}
