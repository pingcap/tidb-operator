...
import (
	...
	"github.com/pingcap/tidb-operator/pkg/controllers/replicationworker/tasks"
...
func Setup(mgr manager.Manager, c client.Client, pdcm pdm.PDClientManager, vm volumes.ModifierFactory) error {
	r := &Reconciler{
		Logger:                mgr.GetLogger().WithName("ReplicationWorker"),
		Client:                c,
		PDClientManager:       pdcm,
		VolumeModifierFactory: vm,
	}
	return ctrl.NewControllerManagedBy(mgr).For(&v1alpha1.ReplicationWorker{}).
		Owns(&corev1.Pod{}).
