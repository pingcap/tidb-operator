...
func newPVCs(rw *v1alpha1.ReplicationWorker) []*corev1.PersistentVolumeClaim {
	pvcs := make([]*corev1.PersistentVolumeClaim, 0, len(rw.Spec.Volumes))
	...
	for i := range rw.Spec.Volumes {
		...
		pvcs = append(pvcs, &corev1.PersistentVolumeClaim{
			ObjectMeta: metav1.ObjectMeta{
				Name:      PersistentVolumeClaimName(coreutil.PodName[scope.ReplicationWorker](rw), vol.Name),
				Namespace: rw.Namespace,
				Labels:    coreutil.PersistentVolumeClaimLabels[scope.ReplicationWorker](rw, vol.Name),
	...
	if rw.Spec.Overlay != nil {
		for _, o := range rw.Spec.Overlay.PersistentVolumeClaims {
