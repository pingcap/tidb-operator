...
func TestPVCNewer(t *testing.T) {
	cases := []struct {
		desc      string
		c         *v1alpha1.Cluster
		obj       *v1alpha1.ReplicationWorker
		enableVAC bool
	...
	}{
		{
			...
			obj: fake.FakeObj("aaa", func(obj *v1alpha1.ReplicationWorker) *v1alpha1.ReplicationWorker {
				return obj
			}),
	...
	for i := range cases {
		...
		t.Run(c.desc, func(tt *testing.T) {
			tt.Parallel()
			pvcs := coreutil.PVCs[scope.ReplicationWorker](c.c, c.obj, coreutil.EnableVAC(c.enableVAC))
