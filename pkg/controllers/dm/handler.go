...
func (r *Reconciler) ClusterEventHandler() handler.TypedEventHandler[client.Object, reconcile.Request] {
	return handler.TypedFuncs[client.Object, reconcile.Request]{
		UpdateFunc: func(ctx context.Context, event event.TypedUpdateEvent[client.Object],
			...
			var dml v1alpha1.DMList
			if err := r.Client.List(ctx, &dml, client.MatchingLabels{
				...
				r.Logger.Error(err, "cannot list all dm instances", "ns", newObj.Namespace, "cluster", newObj.Name)
				return
			}

			for i := range dml.Items {
				dm := &dml.Items[i]
				r.Logger.Info("queue add", "reason", "cluster change", "namespace", dm.Namespace, "cluster", dm.Spec.Cluster, "name", dm.Name)
