...
func createMockStoreInstance(ctrl *gomock.Controller, offline bool, condition []metav1.Condition, annotations map[string]string) *runtime.MockStoreInstance {
	...
	mockStore.EXPECT().Conditions().Return(condition).AnyTimes()
	mockStore.EXPECT().SetConditions(gomock.Any()).AnyTimes()
	if annotations == nil {
...
func TestTaskOfflineStoreStateMachine(t *testing.T) {
	...
	type testCase struct {
		...
		assertFunc         func(t *testing.T, tt testCase, instance *runtime.MockStoreInstance)
		...
		tests := []testCase{
			...
			},
			{
				...
				instanceBuilder: func(ctrl *gomock.Controller) *runtime.MockStoreInstance {
					...
					mockStore.EXPECT().SetOffline(gomock.Any()).AnyTimes()
					mockStore.EXPECT().Conditions().Return(conditionsFromSingle(newOfflineCondition(v1alpha1.OfflineReasonFailed, "", metav1.ConditionTrue))).AnyTimes()
					mockStore.EXPECT().SetConditions(gomock.Any()).AnyTimes()
