...
func TestPVCNewer(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			obj: fake.FakeObj("aaa", func(obj *v1alpha1.Scheduling) *v1alpha1.Scheduling {
				return obj
			}),
	...
	for i := range cases {
		...
		t.Run(c.desc, func(tt *testing.T) {
			tt.Parallel()
			pvcs := coreutil.PVCs[scope.Scheduling](c.c, c.obj, coreutil.EnableVAC(c.enableVAC))
