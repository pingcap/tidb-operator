...
func (r *Reconciler) ClusterEventHandler() handler.TypedEventHandler[client.Object, reconcile.Request] {
	return handler.TypedFuncs[client.Object, reconcile.Request]{
		UpdateFunc: func(ctx context.Context, event event.TypedUpdateEvent[client.Object],
			...
			var sl v1alpha1.SchedulingList
			if err := r.Client.List(ctx, &sl, client.MatchingLabels{
				...
				r.Logger.Error(err, "cannot list all scheduling instances", "ns", newObj.Namespace, "cluster", newObj.Name)
				return
			}

			for i := range sl.Items {
				scheduling := &sl.Items[i]
		...
		DeleteFunc: func(ctx context.Context, event event.TypedDeleteEvent[client.Object],
			...
			r.Logger.Info("enqueue all scheduling instances because of the cluster is deleting", "ns", obj.Namespace, "cluster", obj.Name)
			...
			if err := r.Client.List(ctx, &sl, client.MatchingLabels{
				...
				r.Logger.Error(err, "cannot list all scheduling instances", "ns", obj.Namespace, "cluster", obj.Name)
				return
			}
