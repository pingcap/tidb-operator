...
func TestTaskStatus(t *testing.T) {
	...
	cases := []struct {
		...
		expectedObj    *v1alpha1.TiCDC
	}{
		{
			desc: "no pod but healthy",
			state: &ReconcileContext{
				State: &state{
					ticdc: fake.FakeObj(fakeTiCDCName, func(obj *v1alpha1.TiCDC) *v1alpha1.TiCDC {
						obj.Generation = 3
			...
			expectedObj: fake.FakeObj(fakeTiCDCName, func(obj *v1alpha1.TiCDC) *v1alpha1.TiCDC {
				obj.Generation = 3
				obj.Labels = map[string]string{
					v1alpha1.LabelKeyInstanceRevisionHash: newRevision,
				}
		...
		},
		{
			...
			state: &ReconcileContext{
				State: &state{
					...
					pod: fake.FakeObj("aaa-ticdc-xxx", func(obj *corev1.Pod) *corev1.Pod {
						obj.Labels = map[string]string{
		...
		{
			desc: "pod is ready but ticdc is not healthy",
			state: &ReconcileContext{
				State: &state{
					ticdc: fake.FakeObj(fakeTiCDCName, func(obj *v1alpha1.TiCDC) *v1alpha1.TiCDC {
						obj.Generation = 3
	...
	for i := range cases {
		...
		t.Run(c.desc, func(tt *testing.T) {
			...
			obj := &v1alpha1.TiCDC{}
			require.NoError(tt, fc.Get(ctx, client.ObjectKey{Name: fakeTiCDCName}, obj), c.desc)
