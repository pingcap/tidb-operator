...
func TaskConfigMap(state *ReconcileContext, c client.Client) task.Task {
	return task.NameTaskFunc("ConfigMap", func(ctx context.Context) task.Result {
		...
		state.ConfigHash, err = hasher.GenerateHash(state.TiCDC().Spec.Config)
		if err != nil {
			return task.Fail().With("failed to generate hash for `ticdc.spec.config`: %w", err)
		}
		expected := newConfigMap(state.TiCDC(), data, state.ConfigHash)
...
func newConfigMap(ticdc *v1alpha1.TiCDC, data []byte, hash string) *corev1.ConfigMap {
