	...
	return handler.TypedFuncs[client.Object, reconcile.Request]{
		UpdateFunc: func(ctx context.Context, event event.TypedUpdateEvent[client.Object],
			...
			if err := r.Client.List(ctx, &list, client.MatchingLabels{
				...
				r.Logger.Error(err, "cannot list all router instances", "ns", newObj.Namespace, "cluster", newObj.Name)
				return
			}

			for i := range list.Items {
				router := &list.Items[i]
				queue.Add(reconcile.Request{NamespacedName: types.NamespacedName{Name: router.Name, Namespace: router.Namespace}})
			}
		},
		DeleteFunc: func(ctx context.Context, event event.TypedDeleteEvent[client.Object],
			...
			r.Logger.Info("enqueue all router instances because of the cluster is deleting", "ns", obj.Namespace, "cluster", obj.Name)
			...
			if err := r.Client.List(ctx, &list, client.MatchingLabels{
				...
				r.Logger.Error(err, "cannot list all router instances", "ns", obj.Namespace, "cluster", obj.Name)
				return
			}
