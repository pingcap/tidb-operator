...
import (
	...
	routercfg "github.com/pingcap/tidb-operator/v2/pkg/configs/router"
...
func TaskConfigMap(state *ReconcileContext, c client.Client) task.Task {
	return task.NameTaskFunc("ConfigMap", func(ctx context.Context) task.Result {
		...
		obj := state.Object()
		decoder, encoder := toml.Codec[routercfg.Config]()
		if err := decoder.Decode([]byte(obj.Spec.Config), &cfg); err != nil {
			return task.Fail().With("router config cannot be decoded: %v", err)
		}

		if err := cfg.Overlay(cluster, obj); err != nil {
			return task.Fail().With("cannot generate router config: %v", err)
		}

		data, err := encoder.Encode(&cfg)
		if err != nil {
			return task.Fail().With("router config cannot be encoded: %v", err)
		}

		expected := newConfigMap(obj, data)
		if err := c.Apply(ctx, expected); err != nil {
			return task.Fail().With("can't create/update the cm of router: %v", err)
		}
		return task.Complete().With("cm is synced")
	})
}

func newConfigMap(router *v1alpha1.Router, data []byte) *corev1.ConfigMap {
	return &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      coreutil.PodName[scope.Router](router),
			Namespace: router.Namespace,
			Labels:    coreutil.ConfigMapLabels[scope.Router](router),
