...
import (
	...
	"github.com/pingcap/tidb-operator/v2/pkg/controllers/tikvgroup/tasks"
...
func (r *Reconciler) NewRunner(state *tasks.ReconcileContext, reporter task.TaskReporter) task.TaskRunner {
	runner := task.NewTaskRunner(reporter,
		...
		task.IfBreak(common.CondFeatureGatesIsNotSynced[scope.TiKVGroup](state)),

		// get all tikvs
		common.TaskContextSlice[scope.TiKVGroup](state, r.Client),

		task.IfBreak(common.CondObjectIsDeleting[scope.TiKVGroup](state),
			task.BreakOnWait(tasks.TaskPlacementPolicyRefBlock(state, r.Client)),
		...
		common.TaskGroupStatusSelector[scope.TiKVGroup](state),
		common.TaskGroupConditionSuspended[scope.TiKVGroup](state),
		common.TaskGroupConditionReady[scope.TiKVGroup](state),
		common.TaskGroupConditionSynced[scope.TiKVGroup](state),
		common.TaskStatusRevisionAndReplicas[scope.TiKVGroup](state),
