...
func TestGetStrategyResources(t *testing.T) {
	...
	t.Run("should return empty resources when CPU and Memory are nil", func(t *testing.T) {
		...
		// assert
		assert.Equal(t, resource.Quantity{}, resources.Requests[corev1.ResourceCPU])
		assert.Equal(t, resource.Quantity{}, resources.Requests[corev1.ResourceMemory])
		assert.Equal(t, resource.Quantity{}, resources.Limits[corev1.ResourceCPU])
		assert.Equal(t, resource.Quantity{}, resources.Limits[corev1.ResourceMemory])
	})
