...
func EnsureSubResourcesDeleted(ctx context.Context, c client.Client, tso *v1alpha1.TSO) (wait bool, _ error) {
	wait1, err := k8s.DeleteInstanceSubresource(ctx, c, runtime.FromTSO(tso), &corev1.PodList{})
	if err != nil {
		return false, err
	}
	wait2, err := k8s.DeleteInstanceSubresource(ctx, c, runtime.FromTSO(tso), &corev1.ConfigMapList{})
	if err != nil {
		return false, err
	}
	wait3, err := k8s.DeleteInstanceSubresource(ctx, c, runtime.FromTSO(tso), &corev1.PersistentVolumeClaimList{})
	if err != nil {
		return false, err
	}
