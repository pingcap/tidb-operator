...
import (
	...
	workercfg "github.com/pingcap/tidb-operator/v2/pkg/configs/tikvworker"
...
func TaskConfigMap(state State, c client.Client) task.Task {
	return task.NameTaskFunc("ConfigMap", func(ctx context.Context) task.Result {
		...
		obj := state.Object()
		decoder, encoder := toml.Codec[workercfg.Config]()
		if err := decoder.Decode([]byte(obj.Spec.Config), &cfg); err != nil {
			return task.Fail().With("tikv worker config cannot be decoded: %v", err)
		}

		if err := cfg.Overlay(cluster, obj); err != nil {
			return task.Fail().With("cannot generate tikv worker config: %v", err)
		}

		data, err := encoder.Encode(&cfg)
		if err != nil {
			return task.Fail().With("tikv worker config cannot be encoded: %v", err)
		}

		expected := newConfigMap(obj, data)
		if err := c.Apply(ctx, expected); err != nil {
			return task.Fail().With("can't create/update the cm of tikv worker: %v", err)
		}
		return task.Complete().With("cm is synced")
	})
}

func newConfigMap(w *v1alpha1.TiKVWorker, data []byte) *corev1.ConfigMap {
	return &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      coreutil.PodName[scope.TiKVWorker](w),
			Namespace: w.Namespace,
			Labels:    coreutil.ConfigMapLabels[scope.TiKVWorker](w),
