...
func TaskUpdater(state *ReconcileContext, c client.Client, af tracker.AllocateFactory) task.Task {
	return task.NameTaskFunc("Updater", func(ctx context.Context) task.Result {
		...
		minReadySeconds := coreutil.MinReadySeconds[scope.DMWorkerGroup](dwg)
		retryAfter := coreutil.RetryIfInstancesReadyButNotAvailable[scope.DMWorker](state.InstanceSlice(), minReadySeconds)
		if retryAfter != 0 {
