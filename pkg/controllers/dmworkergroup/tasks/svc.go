...
func TaskService(state State, c client.Client) task.Task {
	return task.NameTaskFunc("Service", func(ctx context.Context) task.Result {
		...
		headless := newHeadlessService(dwg)
		if err := c.Apply(ctx, headless); err != nil {
			return task.Fail().With(fmt.Sprintf("can't create headless service of dm-worker: %v", err))
		}

		return task.Complete().With("headless service of dm-worker has been applied")
	})
}

func newHeadlessService(dwg *v1alpha1.DMWorkerGroup) *corev1.Service {
	ipFamilyPolicy := corev1.IPFamilyPolicyPreferDualStack
	return &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      coreutil.HeadlessServiceName[scope.DMWorkerGroup](dwg),
			...
			Ports: []corev1.ServicePort{
				{
					...
					Port:       coreutil.DMWorkerGroupPort(dwg),
					Protocol:   corev1.ProtocolTCP,
					TargetPort: intstr.FromString(v1alpha1.DMWorkerPortName),
				},
			},
