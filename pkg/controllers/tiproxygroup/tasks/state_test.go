...
func TestState(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			objs: []client.Object{
				fake.FakeObj("aaa", func(obj *v1alpha1.TiProxyGroup) *v1alpha1.TiProxyGroup {
					obj.Spec.Cluster.Name = "aaa"
					return obj
				}),
				fake.FakeObj[v1alpha1.Cluster]("aaa"),
				fake.FakeObj("aaa", func(obj *v1alpha1.TiProxy) *v1alpha1.TiProxy {
			...
			expected: &state{
				...
				updateRevision:  "aaa-tiproxy-759f6c5bdc",
				currentRevision: "aaa-tiproxy-759f6c5bdc",
			},
		},
	}
	...
	for i := range cases {
		...
		t.Run(c.desc, func(tt *testing.T) {
			...
			expected.IFeatureGates = stateutil.NewFeatureGates[scope.TiProxyGroup](expected)

			ctx := context.Background()
			res, done := task.RunTask(ctx, task.Block(
				common.TaskContextObject[scope.TiProxyGroup](s, fc),
				common.TaskContextCluster[scope.TiProxyGroup](s, fc),
				common.TaskContextSlice[scope.TiProxyGroup](s, fc),
				common.TaskRevision[runtime.TiProxyGroupTuple](s, fc),
			))
