...
func TestTaskPod(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			dw: newTestDMWorker("aaa-0", func(dw *v1alpha1.DMWorker) {
				dw.Spec.Overlay = &v1alpha1.Overlay{Pod: &v1alpha1.PodOverlay{ObjectMeta: v1alpha1.ObjectMeta{
		...
		{desc: "pod creation fails", dw: newTestDMWorker("aaa-0"), unexpectedErr: true, wantStatus: task.SFail},
		{desc: "pod update fails", dw: newTestDMWorker("aaa-0"), pod: newPod(fake.FakeObj[v1alpha1.Cluster]("cluster"), newTestDMWorker("aaa-0")), unexpectedErr: true, wantStatus: task.SFail},
	}
	...
	for i := range cases {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			ctx := newDMWorkerReconcileContext(c.dw, cluster, c.pod)
			objs := []client.Object{c.dw, cluster}
			if c.pod != nil {
