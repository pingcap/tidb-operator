...
import (
	...
	dmworkercfg "github.com/pingcap/tidb-operator/v2/pkg/configs/dmworker"
...
func TaskConfigMap(state *ReconcileContext, c client.Client) task.Task {
	return task.NameTaskFunc("ConfigMap", func(ctx context.Context) task.Result {
		cfg := dmworkercfg.Config{}
		decoder, encoder := toml.Codec[dmworkercfg.Config]()
		if err := decoder.Decode([]byte(state.DMWorker().Spec.Config), &cfg); err != nil {
			return task.Fail().With("dm-worker config cannot be decoded: %v", err)
		}
		...
		if err != nil {
			return task.Fail().With("dm-worker config cannot be encoded: %v", err)
		}

		expected := newConfigMap(dw, data)
		if err := c.Apply(ctx, expected); err != nil {
			return task.Fail().With("can't create/update the cm of dm-worker: %v", err)
		}
		return task.Complete().With("cm is synced")
	})
}

func newConfigMap(dw *v1alpha1.DMWorker, data []byte) *corev1.ConfigMap {
	return &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      coreutil.PodName[scope.DMWorker](dw),
			Namespace: dw.Namespace,
			Labels:    coreutil.ConfigMapLabels[scope.DMWorker](dw),
