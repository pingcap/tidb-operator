...
func TaskStatus(state *ReconcileContext, c client.Client) task.Task {
	return task.NameTaskFunc("Status", func(ctx context.Context) task.Result {
		...
		dw := state.DMWorker()
		pod := state.Pod()
		ready := coreutil.IsReady[scope.DMWorker](dw)

		needUpdate = syncSuspendCond(dw) || needUpdate

		needUpdate = compare.SetIfChanged(&dw.Status.ObservedGeneration, dw.Generation) || needUpdate
		needUpdate = compare.SetIfNotEmptyAndChanged(
			&dw.Status.UpdateRevision,
			dw.Labels[v1alpha1.LabelKeyInstanceRevisionHash],
		) || needUpdate

		if ready && pod != nil {
			needUpdate = compare.SetIfNotEmptyAndChanged(
				&dw.Status.CurrentRevision,
				pod.Labels[v1alpha1.LabelKeyInstanceRevisionHash],
			) || needUpdate
		}

		if needUpdate {
			if err := c.Status().Update(ctx, dw); err != nil {
		...
		if !ready {
			...
			return task.Retry(defaultTaskWaitDuration).With("dm-worker is not ready, requeue to retry")
		}
...
func syncSuspendCond(dw *v1alpha1.DMWorker) bool {
	return meta.SetStatusCondition(&dw.Status.Conditions, metav1.Condition{
		Type:               v1alpha1.CondSuspended,
		Status:             metav1.ConditionFalse,
		ObservedGeneration: dw.Generation,
