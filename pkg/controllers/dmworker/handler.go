...
func (r *Reconciler) ClusterEventHandler() handler.TypedEventHandler[client.Object, reconcile.Request] {
	return handler.TypedFuncs[client.Object, reconcile.Request]{
		UpdateFunc: func(ctx context.Context, event event.TypedUpdateEvent[client.Object],
			...
			if newObj.Status.ID == oldObj.Status.ID &&
				reflect.DeepEqual(oldObj.Spec.SuspendAction, newObj.Spec.SuspendAction) &&
				oldObj.Spec.Paused == newObj.Spec.Paused {
				return
			}

			var dwl v1alpha1.DMWorkerList
			if err := r.Client.List(ctx, &dwl, client.MatchingLabels{
				...
				r.Logger.Error(err, "cannot list all dm-worker instances", "ns", newObj.Namespace, "cluster", newObj.Name)
				return
			}

			for i := range dwl.Items {
				dw := &dwl.Items[i]
