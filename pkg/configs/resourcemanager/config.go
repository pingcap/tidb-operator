...
// TODO: reduce duplicated code with tso/scheduling configs.
type Config struct {
...
func (c *Config) Overlay(cluster *v1alpha1.Cluster, rm *v1alpha1.ResourceManager) error {
	if err := c.Validate(); err != nil {
		return err
	}

	if coreutil.IsTLSClusterEnabled(cluster) {
		c.Security.CACertPath = path.Join(v1alpha1.DirPathClusterTLSResourceManager, corev1.ServiceAccountRootCAKey)
		c.Security.CertPath = path.Join(v1alpha1.DirPathClusterTLSResourceManager, corev1.TLSCertKey)
		c.Security.KeyPath = path.Join(v1alpha1.DirPathClusterTLSResourceManager, corev1.TLSPrivateKeyKey)
	}

	c.Name = rm.Name
	c.ListenAddr = coreutil.ListenURL(cluster, coreutil.ResourceManagerClientPort(rm))
	c.AdvertiseListenAddr = coreutil.InstanceAdvertiseURL[scope.ResourceManager](cluster, rm, coreutil.ResourceManagerClientPort(rm))
