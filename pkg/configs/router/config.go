...
func (c *Config) Overlay(cluster *v1alpha1.Cluster, r *v1alpha1.Router) error {
	if err := c.Validate(); err != nil {
		return err
	}

	if coreutil.IsTLSClusterEnabled(cluster) {
		c.Security.CACertPath = path.Join(v1alpha1.DirPathClusterTLSRouter, corev1.ServiceAccountRootCAKey)
		c.Security.CertPath = path.Join(v1alpha1.DirPathClusterTLSRouter, corev1.TLSCertKey)
		c.Security.KeyPath = path.Join(v1alpha1.DirPathClusterTLSRouter, corev1.TLSPrivateKeyKey)
	}

	c.Name = r.Name
	c.ListenAddr = coreutil.ListenURL(cluster, coreutil.RouterClientPort(r))
	c.AdvertiseListenAddr = coreutil.InstanceAdvertiseURL[scope.Router](cluster, r, coreutil.RouterClientPort(r))
