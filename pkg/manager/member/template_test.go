...
`,
		...
		{
			name:         "non-empty discoveryUrl",
			scheme:       "http",
			dataSubDir:   "data",
			discoveryUrl: "discovery-server.default.svc.cluster2.local:10261",
			result: `#!/bin/sh
...
cluster_name=` + "`" + `echo ${PEER_SERVICE_NAME} | sed 's/-pd-peer//'` + "`" + `
domain="${POD_NAME}.${PEER_SERVICE_NAME}.${NAMESPACE}.svc"
discovery_url="discovery-server.default.svc.cluster2.local:10261"
encoded_domain_url=` + "`" + `echo ${domain}:2380 | base64 | tr "\n" " " | sed "s/ //g"` + "`" + `
elapseTime=0
period=1
threshold=30
