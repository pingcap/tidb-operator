...
func (p *pvcResizer) buildContextForTC(tc *v1alpha1.TidbCluster, status v1alpha1.ComponentStatus) (*componentVolumeContext, error) {
	...
	case v1alpha1.PDMSTSOMemberType, v1alpha1.PDMSSchedulingMemberType:
		...
		for _, component := range tc.Spec.PDMS {
			if strings.Contains(comp.String(), component.Name) {
				if tc.Status.PDMS[comp.String()].Volumes == nil {
					tc.Status.PDMS[comp.String()].Volumes = map[v1alpha1.StorageVolumeName]*v1alpha1.StorageVolumeStatus{}
				}
			...
			if store.PodName == resizePod.Name {
				...
				if status, exist := tc.Status.TiKV.EvictLeader[resizePod.Name]; exist && !status.BeginTime.IsZero() {
					...
					if time.Since(status.BeginTime.Time) > timeout {
						klog.Infof("%s: leader eviction begins at %q but timeout (threshold: %v)", logPrefix, status.BeginTime.Format(time.RFC3339), timeout)
						return nil
					}
				}
