...
func (tku *tikvUpgrader) Upgrade(tc *v1alpha1.TidbCluster, oldSet *apps.StatefulSet, newSet *apps.StatefulSet) error {
	...
	if oldSet.Spec.UpdateStrategy.Type == apps.OnDeleteStatefulSetStrategyType || oldSet.Spec.UpdateStrategy.RollingUpdate == nil {
		// Manually bypass tidb-operator to modify statefulset directly, such as modify tikv statefulset's RollingUpdate policy to OnDelete policy,
		...
		// Therefore, in the production environment, we should try to avoid modifying the tikv statefulset upgrade strategy directly.
		glog.Warningf("tidbcluster: [%s/%s] tikv statefulset's %s UpdateStrategy has been modified manually", ns, tcName, oldSet.GetName())
		return nil
	}
