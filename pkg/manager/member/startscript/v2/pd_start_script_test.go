...
`,
		...
		{
			name: "BashShebang feature flag",
			modifyTC: func(tc *v1alpha1.TidbCluster) {
				tc.Spec.StartScriptV2FeatureFlags = []v1alpha1.StartScriptV2FeatureFlag{
					v1alpha1.StartScriptV2FeatureFlagBashShebang,
				}
			},
			expectScript: `#!/bin/bash
...
`,
		...
		{
			name: "NoWaitDNS feature flag",
			modifyTC: func(tc *v1alpha1.TidbCluster) {
				tc.Spec.StartScriptV2FeatureFlags = []v1alpha1.StartScriptV2FeatureFlag{
					v1alpha1.StartScriptV2FeatureFlagNoWaitDNS,
				}
			},
			expectScript: `#!/bin/sh
...
`,
		...
		{
			name: "BashShebang and NoWaitDNS feature flags",
			modifyTC: func(tc *v1alpha1.TidbCluster) {
				tc.Spec.StartScriptV2FeatureFlags = []v1alpha1.StartScriptV2FeatureFlag{
					v1alpha1.StartScriptV2FeatureFlagBashShebang,
					v1alpha1.StartScriptV2FeatureFlagNoWaitDNS,
...
if [[ -f /var/lib/pd/join ]]; then
    ...
    until result=$(wget -qO- -T 3 --ca-certificate=/var/lib/discovery-tls/ca.crt --certificate=/var/lib/discovery-tls/tls.crt --private-key=/var/lib/discovery-tls/tls.key https://start-script-test-discovery.start-script-test-ns:10261/new/${encoded_domain_url} 2>/dev/null); do
        echo "waiting for discovery service to return start args ..."
        sleep $((RANDOM % 5))
    done
    ARGS="${ARGS} ${result}"
