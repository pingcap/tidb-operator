...
func newAzblobStorage(conf *azblobConfig) (*blob.Bucket, error) {
	...
	if len(conf.endpoint) > 0 {
		...
		// StorageDomain should be just the base domain (e.g., "blob.core.windows.net")
		// NOT the full hostname with account (e.g., "tibkdevwestus.blob.core.windows.net")
		// because gocloud.dev will prepend the account name automatically
		// Extract domain suffix by removing the account name prefix
		if idx := strings.Index(domain, "."); idx != -1 {
			domain = domain[idx+1:] // Remove "tibkdevwestus." to get "blob.core.windows.net"
			klog.Infof("StorageDomain after removing account name prefix: %s", domain)
		}
...
func newAzblobStorageUsingAAD(conf *azblobConfig, cred *azblobAADCred, azureOptions *azureblob.Options) (*blob.Bucket, error) {
	...
	if len(conf.endpoint) > 0 {
		...
		klog.Infof("Azure endpoint specified: %s", endpoint)
