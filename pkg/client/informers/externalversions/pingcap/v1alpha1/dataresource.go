...
// DataResourceInformer provides access to a shared informer and lister for
// DataResources.
type DataResourceInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.DataResourceLister
}

type dataResourceInformer struct {
	factory          internalinterfaces.SharedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
	namespace        string
}

// NewDataResourceInformer constructs a new informer for DataResource type.
...
func NewDataResourceInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredDataResourceInformer(client, namespace, resyncPeriod, indexers, nil)
}

// NewFilteredDataResourceInformer constructs a new informer for DataResource type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.PingcapV1alpha1().DataResources(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.PingcapV1alpha1().DataResources(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *dataResourceInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredDataResourceInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}
...
func (f *dataResourceInformer) Lister() v1alpha1.DataResourceLister {
	return v1alpha1.NewDataResourceLister(f.Informer().GetIndexer())
}
