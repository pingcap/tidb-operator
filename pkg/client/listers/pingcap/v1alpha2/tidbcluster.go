...
type TidbClusterLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.TidbCluster, err error)
...
// List lists all TidbClusters in the indexer.
func (s *tidbClusterLister) List(selector labels.Selector) (ret []*v1alpha2.TidbCluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.TidbCluster))
	})
	return ret, err
}
...
type TidbClusterNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.TidbCluster, error)
...
// List lists all TidbClusters in the indexer for a given namespace.
func (s tidbClusterNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.TidbCluster, err error) {
...
// Get retrieves the TidbCluster from the indexer for a given namespace and name.
func (s tidbClusterNamespaceLister) Get(name string) (*v1alpha2.TidbCluster, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("tidbcluster"), name)
	}
	return obj.(*v1alpha2.TidbCluster), nil
}
